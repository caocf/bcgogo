...
//		sendBroadcast(intentS);
	...
	private void bindObd(final String vehicleVin, final String vehicleNo, final String vehicleModel,
			final String vehicleModelId, final String vehicleBrand, final String vehicleBrandId, final String obdSN,
			final String userNo, final String nextMaintainMileage) {
		showLoadingDialog("保存中...");
		...
		request.doRequest(this, new AsyncJsonBaseResponseParseHandler<VehicleResponse>() {
			...
			public void onParseSuccess(VehicleResponse result, String originResult) {
				...
				VehicleInfo vehicle = result.getVehicleInfo();
//				vehicle.setVehicleVin(vehicleVin);
//				vehicle.setVehicleNo(vehicleNo);
//				vehicle.setVehicleModel(vehicleModel);
//				vehicle.setVehicleModelId(vehicleModelId);
//				vehicle.setVehicleBrandId(vehicleBrandId);
//				vehicle.setVehicleBrand(vehicleBrand);
//				vehicle.setUserNo(userNo);
//				vehicle.setObdSN(obdSN);
//				vehicle.setNextMaintainMileage(nextMaintainMileage);
				List<VehicleInfo> vehicles = TongGouApplication.sVehicleList;
				if( vehicles == null || vehicles.isEmpty()) {
					vehicle.setIsDefault("YES");
					List<VehicleInfo> newVehicleData = new ArrayList<VehicleInfo>();
					newVehicleData.add(vehicle);
					TongGouApplication.getInstance().setDefaultVehicleBindOBDs(newVehicleData);
				} else {
					vehicle.setIsDefault("NO");
					TongGouApplication.sVehicleList.add(vehicle);
				}
				
				TongGouApplication.getInstance().queryVehicleList(TongGouApplication.getInstance());
				BaseConnectOBDService.addingCar = false;
				finish();
			}
	...
	// 保存车辆信息
	private void saveVehicleInfo(final String vehicleId, final String vehicleVin, final String vehicleNo,
			final String vehicleModel, final String vehicleModelId, final String vehicleBrand, final String vehicleBrandId,
			final String userNo) {
		showLoadingDialog("保存中...");
