...
public class SomeUtil {
	...
	public static final long HOUR_RATE = 1000 * 60 * 60;
	public static final long MINUTE_RATE = 1000 * 60;
	...
	public static int[] getSpanParams(GeoPoint[] geoPoints) {
		if (geoPoints == null || geoPoints.length < 1) {
			return null;
		}
		return sGetSpanParams(geoPoints);
	}

	private static int[] sGetSpanParams(GeoPoint[] geoPoints) {
	 ...
	 * @return eg:"1月"
	 */
	public static String getMonth2(String strDate) {
		return strDate.substring(strDate.indexOf("-") + 1, strDate.indexOf("-") + 3) + "";
	}
	 ...
	 * @return "2/13~2/19"
	 */
	public static String getWeekPattern(String startDay, String endDay) {
		return getMonth2(startDay) + "/" + getDay(startDay) + "~" + getMonth2(endDay) + "/" + getDay(endDay);
	}
	...
	public static String fmtMilliseconds(long millScd) {
		if (millScd / HOUR_RATE >= 1) {
			long hours = (millScd) / HOUR_RATE;
			long minutes = (millScd % HOUR_RATE) / MINUTE_RATE;
	...
	public static double fmtDouble(Double d) {
		return baseFmtDouble(d, 1);
	}

	public static double fmtDouble(String str) {
		return fmtDouble(stringToDouble(str));
	}

	public static double baseFmtDouble(Double d, int decimalPlace) {
		if (decimalPlace < 0) {
			return d;
		}
		try {
			BigDecimal bigDecimal = new BigDecimal(d).setScale(decimalPlace, RoundingMode.HALF_UP);
	...
	public static double baseFmtDouble(String str, int decimalPlace) {
		return baseFmtDouble(stringToDouble(str), decimalPlace);
	}

	public static final boolean isGPSOn(final Context context) {

		if (hasSystemFeatureGPS(context)) {
	 ...
	 */
	public static final boolean hasSystemFeatureGPS(final Context context) {
		return context.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LOCATION_GPS);
	}

	public static double getOilWear(double avgOilWear, double distance) {
		if (avgOilWear > 0 && distance > 0) {
			return SomeUtil.fmtDouble(avgOilWear * (distance / 100));
		}
		return 0d;
	}

	public static double getOilCost(double avgOilWear, double oilPrice, double distance) {
		if (avgOilWear > 0 && oilPrice > 0 && distance > 0) {
			return SomeUtil.fmtDouble(oilPrice * getOilWear(avgOilWear, distance));
		}
		return 0d;
	}
