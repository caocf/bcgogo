...
public class DJDatabase {
	...
	private ArrayList<DrivingJournalItem> queryDJItems(String selection, String[] selectionArgs) {
		...
		String orderBy = FIELD_STARTTIME + " desc";
	...
	public ArrayList<DrivingJournalItem> queryNotUploadDJItems() {
		String selection = FIELD_VEHICLENO + " =?" + AND + FIELD_STATUS + " =?" + AND + FIELD_STARTPLACE
				+ " IS NOT NULL" + AND + FIELD_ENDPLACE + " IS NOT NULL";
	...
	public DrivingJournalItem queryLastDJItem() {
		String vehiNo = getVehiNo();
		if (vehiNo == null) {
			return null;
		}
		DrivingJournalItem djItem = null;
		String selection = FIELD_VEHICLENO + " =?";
		String selectionArgs[] = new String[] {vehiNo};
		Cursor query = mRDatabase.rawQuery("SELECT * FROM " + TABLE_DRIVING_JOURNAL_ITEM + " WHERE "
				+ selection + " ORDER BY " + FIELD_STARTTIME + " DESC LIMIT 1", selectionArgs);
		if (query != null) {
			if (query.moveToNext()) {
				djItem = createDJItem(query);
			}
			query.close();
		}
		return djItem;
