...
public class BaseConnectOBDService extends Service{
	...
	public static String ACTION_ON_GET_DTC_DATAS = "com.tonggou.andclient.app.BaseConnectOBDService.AOGDD";
	public static final String ACTION_WRITE_MILE_SUCCESS = "com.tonggou.andclient.writeMileSuccess";
	public static final String EXTRA_CURRENT_MILE = "extra_current_mile"; // 里程写成功后返回的总里程
	public static final String EXTRA_BATTERY_VOLTAGE = "extra_battery_voltage"; // 电瓶电压
	public static final String EXTRA_OIL_TANK_TEMPERATURE = "extra_oil_tank_temperature"; // 水箱温度
	public static final String EXTRA_AVG_OIL_WEAR = "extra_oil_avg_oil_wear"; // 平均油耗，也就是百公里油耗
	public static final String EXTRA_TOTAL_DISTANCE = "extra_total_distance"; // 总里程
...
//		devices = new ArrayList<BluetoothDevice>();
		...
		for (int i = 0; i < TongGouApplication.sObdLists.size(); i++) {
			if ("YES".equals(TongGouApplication.sObdLists.get(i).getIsDefault())) {
				currentDefaultObdSn = TongGouApplication.sObdLists.get(i).getObdSN(); // 找到服务器送下来的默认obd
				break;
			}
		}
...
//						bBindNoChange = true;
//					}
			cmile = obdInfo.getVehicleInfo().getCurrentMileage();
...
//			}
	...
	private void writeCurrentMile(final String miles){
		new Thread(){
			public void run(){
				String mil = TextUtils.isEmpty(miles) ? "0" : miles; 
                String writeMiles = "##MIL:"+mil+"\r\n";
                ...
                if(readMessage!=null){
                	...
                	if(readMessage.indexOf("VIN:")!=-1&&readMessage.length()>4){
                		...
                		if(readMessage.indexOf("OK")!=-1){              			
                			Intent intent = new Intent(ACTION_WRITE_MILE_SUCCESS);
                			intent.putExtra(EXTRA_CURRENT_MILE, cmile);
...
//	    }
	    ...
	    private void prosseingOBDError(final String codeStrs){	  
	    	...
	    	for(int i=0;i<errors.length;i++){
	    		...
	    		if(modleFaults!=null&&modleFaults.size()>0){
		    		...
		    		oneConditionMessage.getFaultCodeInfo().setBackgroundInfo( "不能识别的故障码" );
	    		}
	    		
	    		int resultFlag = SaveDB.getSaveDB(this).saveAlarm(oneConditionMessage);   //保存到数据库	
	    		if( resultFlag > 0 ) {
	    			newErrorCount++;
	    		}
	    	}
	    	...
	    	if(newErrorCount>0){//提示用户
				if ( TongGouApplication.getInstance().isLogin()) {  	    		//如果是登录状态						
	    ...
	    private void sendReceivedOBDDTCBroadcast() {
	    	...
	    	intent.setAction(ACTION_ON_GET_DTC_DATAS);
	    	sendBroadcast(intent);
	    }
		...
		private void prosseingCurrentConditon(String codeStrs) {
			...
			// OBD端回复：##瞬时油耗（ml/s），百公里油耗（l/100km）,油耗(l/h),
			// 油量（%）,里程（km），发动机水温（℃），电瓶电压（V）\r\n
			...
			if ("N/A".equals(ssyh)) {
				...
				try {
					...
					double ssyhHour = (ssyhDouble * 3600) / 1000; // 瞬时油耗 换算ml/s ->
																	// l/h 用于显示
...
//			}
			...
			intent.putExtra(EXTRA_BATTERY_VOLTAGE, dianya);
			intent.putExtra(EXTRA_OIL_TANK_TEMPERATURE, sxwd);
			intent.putExtra(EXTRA_AVG_OIL_WEAR, pjyh);
			intent.putExtra(EXTRA_TOTAL_DISTANCE, licheng);
			sendBroadcast(intent);
			
//			Intent intent2 = new Intent(DrivingJournalService.ACTION_UPDATE_VEHICLE_CONDITION);
//			intent2.putExtra(DrivingJournalService.EXTRA_CURR_BATTERY_VOLTAGE, dianya);
//			intent2.putExtra(DrivingJournalService.EXTRA_CURR_TANK_TEMPERATURE, sxwd);
//			intent2.putExtra(DrivingJournalService.EXTRA_CURR_AVG_OIL_WEAR, pjyh);
//			intent2.putExtra(DrivingJournalService.EXTRA_CURR_ON_ROAD_DISTANCE, licheng);
