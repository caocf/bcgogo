...
import com.tonggou.andclient.network.request.QuerySuportAreaListRequest;
...
//	private ProgressBar progressBar;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		array = findViewById(R.id.sort_btn_container);
		arraySet= findViewById(R.id.sort_btn);
		arraySet.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				if(pw != null){
					if(pw.isShowing()){	
						pw.dismiss();
			...
			public void onClick(View v) {
				if(pw != null){
					if(pw.isShowing()){
						...
						//先加载城市列表信息，加载成功后显示列表
						getProvinces(AreaType.PROVINCE, null);
						//						
				...
				case NETWORK_PROVINCE_SUCCEED: 
					...
					if(locationProvinceName!=null&&!"".equals(locationProvinceName)){
						for(int i=0;i<province.size();i++){
							if(locationProvinceName.equals(province.get(i).getName())){
								...
								proviceNum = i;  //选中位置
								getProvinces(AreaType.CITY, String.valueOf(thisProvinceId));
							}
						}
					}else{
					...
					lv_group.setOnItemClickListener(new OnItemClickListener() {  
						public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) { 
							...
							proviceNum=position;
							getProvinces(AreaType.CITY, String.valueOf(provinceId));
							adapter.notifyDataSetChanged();
						}  
					});
					break;
...
//					serviceScopeIds=ids.toString().substring(0, ids.toString().length()-1);
		...
		chooseSortName = (Button)arraySet;
...
//	}
	...
	public void doQueryStore(String serviceScopeIds, String areaId, final ShopType shopType, String keyword, int pageNo, int pageSize, final boolean isRefresh) {
		showLoadingDialog("加载中...");
		nextPageLock = true;
		final QueryShopListRequest request = new QueryShopListRequest();
		...
		request.doRequest(this, new AsyncLoadCacheJsonBaseResponseParseHandler<QueryShopResponse>() {

			@Override
			public void onParseSuccess(QueryShopResponse result, String originResult) {
			...
			@Override
			public void onLoadCache(QueryShopResponse result, String originResult,
					boolean isNetworkConnected) {
				if( result != null && !mIsMore) {
					ArrayList<Shop> shops = (ArrayList<Shop>)result.getShopList();
					if( shops != null ) {
						updateShopData( shops, true );
					}
				}
	...
	private void getSearchShopAction(String key,String cityCode,String areaid){
		...
		request.doRequest(this, new AsyncJsonBaseResponseParseHandler<ShopSuggestionResponse>() {

			@Override
			public void onParseSuccess(ShopSuggestionResponse result, String originResult) {
				super.onParseSuccess(result, originResult);
...
//				super.onParseFailure(errorCode, errorMsg);
	...
	private void dismissPopupWindow(PopupWindow pw) {
	...
	private void getProvinces(final AreaType type,final String provinceId) {
		QuerySuportAreaListRequest request = new QuerySuportAreaListRequest();
		request.setApiParams(type, provinceId);
		request.doRequest(this, new AsyncLoadCacheJsonBaseResponseParseHandler<PlaceResponse>() {

			@Override
			public void onLoadCache(PlaceResponse result, String originResult,
					boolean isNetworkConnected) {
				if( result != null ) {
					if(type == AreaType.PROVINCE){
						province = result.getAreaList();
						if(province!=null&&province.size()>0){
							showProvincePop();
						}				
					}else{
						city = result.getAreaList();
						if(city!=null){
							displayCity();
						}	
					}
			...
			@Override
			public void onParseSuccess(PlaceResponse result, String originResult) {
				super.onParseSuccess(result, originResult);
				if(type == AreaType.PROVINCE){
					province = result.getAreaList();
					if(province!=null&&province.size()>0){
			...
			@Override
			public Class<PlaceResponse> getTypeClass() {
	...
	private void requestServiceScope(ServiceScopeType type) {
		...
		request.doRequest(this, new AsyncLoadCacheJsonBaseResponseParseHandler<ServiceCategoryResponse>() {
			...
			public void onParseSuccess(ServiceCategoryResponse result, String originResult) {
				super.onParseSuccess(result, originResult);
				if( servicesCategory == null || servicesCategory.isEmpty() ) {
