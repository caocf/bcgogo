		...
		mToBindView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(TongGouApplication.getInstance().isLogin()) {
					Intent intent = new Intent(CarConditionQueryActivity.this, ConnectOBDDialogActivity.class);
					intent.putExtra("tonggou.connectobd.from", "fromSettings");
					startActivityForResult(intent, 123);
				} else {
					Intent intent = new Intent(CarConditionQueryActivity.this, LoginActivity.class);
...
//				startActivity(intent);
	...
	private void deleteAlarm( final String faultCode ) {
		...
		new Thread() {
			...
			public void run() {
				if( faultCode != null ) {
					SaveDB.getSaveDB(CarConditionQueryActivity.this).deleteOneAlarm(currentUsername, faultCode, vehicleId );
					getMessage();
				} else {
					boolean isSuccess = SaveDB.getSaveDB(CarConditionQueryActivity.this).deleteAllAlarm(currentUsername, vehicleId);
				    if(isSuccess){
	...
	public void getMessage() {
		...
		List<CarCondition> data = SaveDB.getSaveDB(CarConditionQueryActivity.this).getAllCarConditons(currentUsername, vehicleId);
	...
	class CarConditionAdapter extends AbsViewHolderAdapter<CarCondition> {
	 ...
	 class UPDateUiReceiver extends BroadcastReceiver{
        ...
        public void onReceive(Context context, Intent intent) {
        	if( !TongGouService.TONGGOU_ACTION_UPDATEUI.equals( intent.getAction()) ) {
        		return;
        	}
