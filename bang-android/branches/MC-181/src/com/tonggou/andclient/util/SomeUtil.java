...
public class SomeUtil {
	...
	public static String fmtMilliseconds(long millScd) {
		...
		return millScd * HOUR_RATE > 1 ? dfmt.format(millScd * HOUR_RATE) + HOUR_UNIT : dfmt.format(millScd
				* MINUTE_RATE)
				+ MINUTE_UNIT;
	}
	...
	public static double getFmtDouble(Double d) {
		return getFmtDouble(d, 1);
	}

	public static double getFmtDouble(String str) {
		try {
			return getFmtDouble(getDouble(str));
		} catch (Exception e) {
			return 0d;
		}
	}

	public static double getFmtDouble(Double d, int decimalPlace) {
		DecimalFormat dfmt;
		if (decimalPlace == 0) {
			...
			dfmt = new DecimalFormat("0." + builder.toString());
		}
		try {
			return (Double.parseDouble(dfmt.format(d)));
		} catch (Exception e) {
			return 0d;
		}
	}

	public static double getFmtDouble(String str, int decimalPlace) {
		return getFmtDouble(getDouble(str), decimalPlace);
	}
