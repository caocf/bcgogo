...
import com.tonggou.andclient.network.request.LogoutRequest;
...
// @BEGIN	
// 需求修改，不需要断开 OBD ,在 资源文件中已经设置为 gone,
		/*
		 *  @add
	...
	private void doCheckUpdate() {
		...
		updateCheck.doCheckUpdate(updateCheck.new OnUpdateCheckListener() {
			...
			public void onNothingToUpdate() {
				showSuccessMessageDialog("已是最新版本");
			}
...
//		sendBroadcast(intentS);
	...
	private void logout() {
		...
		cancleAllRequest();
		TongGouApplication.getInstance().stopPollingMsg();
		Thread.yield();
		...
		request.doRequest(this, new AsyncJSONResponseParseHandler<BaseResponse>() {
			...
			public void onParseFailure(String errorCode, String errorMsg) {
				...
				TongGouApplication.getInstance().startPollingMsg();
			}
