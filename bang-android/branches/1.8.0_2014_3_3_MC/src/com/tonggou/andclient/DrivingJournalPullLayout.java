...
public class DrivingJournalPullLayout extends LinearLayout {
	...
	private FrameLayout mFlDrag, mFlCalendar;
	...
	private void initViews() {
		...
		mFlCalendar = (FrameLayout) mPullLayout.findViewById(R.id.fl_driving_journal_calendar);
		...
		mLlCalendar = (LinearLayout) findViewById(R.id.ll_driving_journal_calendar);
		mTxtNoData = (TextView) findViewById(R.id.txt_driving_journal_calendar_no_data);

		mPullLayout.findViewById(R.id.fl_driving_journal_drag).setOnClickListener(mOnClickListener);
		if (DJDatabase.getInstance(mContext).queryLastDJItem() == null) {
	...
	private void setVisible(int v1, int v2) {
		mLlCalendar.setVisibility(v1);
		mTxtNoData.setVisibility(v2);
	}
...
//			Log.d(TAG, "CurrStatus After£º" + mCurrStatus.toString());
	...
	private void startExpandCalendarAnim() {
		...
		mFlCalendar.setAnimation(alphaAnimation);
	...
	private void changeOthersParams(boolean changeToInit) {
		...
		if (changeToInit) {
			...
			mPullLayout.setLayoutParams(pullrParams);
			mLlOhters.setBackgroundResource(R.drawable.bg_driving_journal_calendar);
		}
	}
}
