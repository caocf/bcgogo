...
public class BindCarsActivity extends BaseActivity{
				...
				if (arg2 <vehicleList.size()) {
					...
					.setPositiveButton(getString(R.string.button_ok), new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int id) {
							...
							new Thread(){
								public void run(){                               
									networkingDelete(vehicleList.get(position).getVehicleId(),vehicleList.get(position).getIsDefault(),vehicleList.get(position).getVehicleModelId());
								}
							}.start();
						}
	...
	private  void networkingDelete(String veId,String ifdefault,String modelId){ 
		...
		if(ns.isNetworkSuccess()){
			if(deletevehicleParser.isSuccessfull()){
				...
				//É¾³ý×Öµä
				SaveDB.getSaveDB(BindCarsActivity.this).deleteModleFaultCodes(currentUserId+modelId);
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(resultCode==3030||resultCode==5050){		
			//ÇëÇóÍøÂç
