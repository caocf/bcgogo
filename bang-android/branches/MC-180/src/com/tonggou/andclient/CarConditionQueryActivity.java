...
//				startActivity(intent);
	...
	private void deleteAlarm( final String faultCode ) {
		new Thread() {
			...
			public void run() {
				if( faultCode != null ) {
					SaveDB.getSaveDB(CarConditionQueryActivity.this).deleteOneAlarm(currentUsername, faultCode);
					getMessage();
				} else {
					boolean isSuccess = SaveDB.getSaveDB(CarConditionQueryActivity.this).deleteAllAlarm(currentUsername);
				    if(isSuccess){
	...
	public void getMessage() {
		List<CarCondition> data = SaveDB.getSaveDB(CarConditionQueryActivity.this).getAllCarConditons(currentUsername);
