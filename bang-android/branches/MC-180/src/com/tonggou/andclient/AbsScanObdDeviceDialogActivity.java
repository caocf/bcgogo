...
import com.tonggou.andclient.util.BluetoothUtil;
...
//        TongGouApplication.getInstance().queryVehicleList();
 		...
 		Intent intentS = new Intent();
 		intentS.setAction(TongGouService.TONGGOU_ACTION_START);
 		intentS.putExtra("com.tonggou.server","SCAN_OBD");
 		sendBroadcast(intentS);
	...
	public void onConnectSuccess(OBDDevice device) {
		mRemoteDevice = mBtAdapter.getRemoteDevice(device.getDeviceAddress());
	...
	private void removeDevice() {
		if( mRemoteDevice != null ) {
			try {
				...
				BluetoothUtil.removeBond(mRemoteDevice);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
