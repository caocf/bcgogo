...
public class CarErrorActivity extends BaseActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		new Thread(){
			public void run(){
				ArrayList<CarCondition> havealarms = SaveDB.getSaveDB(CarErrorActivity.this).getAllCarConditons(currentUsername);		
	...
	protected void showWrongAlert(final CarCondition carCondition){
		...
		retryOper.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				wrongAlert.cancel();
				wrongAlert.dismiss();			
				SaveDB.getSaveDB(CarErrorActivity.this).readedTimeAlarm(currentUsername,carCondition.getFaultCode(),System.currentTimeMillis()+"");
				SaveDB.getSaveDB(CarErrorActivity.this).readedOneAlarm(currentUsername,carCondition.getFaultCode());
			}
		});
