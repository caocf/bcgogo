 ...
 *
 */
public class VideoRefreshViewLoadMoreProxy implements OnRefreshListener2<View>{

	private PollingStatus status;

	public PollingStatus isPullingDownMode() {
		return status;
	}
	...
	public VideoRefreshViewLoadMoreProxy(PullToRefreshAdapterViewBase pullToRefreshView) {
		this(pullToRefreshView, Mode.BOTH, 0, 0);
	}
	
	public VideoRefreshViewLoadMoreProxy(PullToRefreshAdapterViewBase pullToRefreshView, Mode mode) {
		this(pullToRefreshView, mode, 0, 0);
	}

	public VideoRefreshViewLoadMoreProxy(PullToRefreshAdapterViewBase pullToRefreshView, int startPage, int currentPage) {
		this(pullToRefreshView, Mode.BOTH, startPage, currentPage);
	}

	public VideoRefreshViewLoadMoreProxy(PullToRefreshAdapterViewBase pullToRefreshView, Mode mode, int startPage, int currentPage) {
	...
	public void setFromStartLabel(int pullLabelRes, int releaseLabelRes) {
		mPullToRefreshView.getLoadingLayoutProxy(true, false).setPullLabel(getString(pullLabelRes));
		mPullToRefreshView.getLoadingLayoutProxy(true, false).setReleaseLabel(getString(releaseLabelRes));
	}

	public void setFromEndLabel(int pullLabelRes, int releaseLabelRes) {
		mPullToRefreshView.getLoadingLayoutProxy(false, true).setPullLabel(getString(pullLabelRes));
		mPullToRefreshView.getLoadingLayoutProxy(false, true).setReleaseLabel(getString(releaseLabelRes));
	}
	 ...
	 * 加载数据完毕调用此方法
	 * 
	 * @param isRefresh 是否为刷新.true 刷新，false 不是刷新。该参数只有在需要更新时间时才有效
	 */
