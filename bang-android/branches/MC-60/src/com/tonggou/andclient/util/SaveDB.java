...
public class SaveDB {
	...
	//////////////////////////////故障码相关版本
	private static String FAULT_DIC_MODLE = "modle";       //车型id 或者是 common字符串标识
	private static String FAULT_DIC_VERSION = "version";   //版本

	
	private static final int DIC_DATABASE_VERSION = 1;
    private static final String DIC_VER_TABLE_NAME = "faultversion";
    private static final String DIC_VER_TIONARY_TABLE_CREATE = 
    	"CREATE TABLE " + DIC_VER_TABLE_NAME + " (" +
    	FAULT_DIC_MODLE + " TEXT, " +
    	FAULT_DIC_VERSION + " TEXT);";
    
    private static final String[] DIC_VER_TABLE_CREATE2 = {FAULT_DIC_MODLE,FAULT_DIC_VERSION};
    //private FaultVerDbHelper faultVerHelper;
    ////////////////////////////////////////////////////////////////
	
	
	//////////////////////////////故障码相关
	private static String FAULT_TYPT = "faulttype";       //车型id 或者是 通用common字符串标识
	private static String FAULT_CODE = "faultcode";       //错误码
	private static String FAULT_DESCRIPTION = "description";    //描述
	
	private static final int DATABASE_VERSION = 1;
    private static final String DICTIONARY_TABLE_NAME = "faultcodes";
    ...
    private static final String[] DICTIONARY_TABLE_CREATE2 = {FAULT_TYPT,FAULT_CODE,FAULT_DESCRIPTION};
    ...
    private static final String MESSAGE_TABLE_CREATE = 
    	...
    	MESSAGE_ACTION_TYPE + " TEXT, " +
    	MESSAGE_SEARCH_SHOP + " TEXT, " +
    	MESSAGE_SERVICE_DETAIL + " TEXT, " +
    	MESSAGE_CANCEL_ORDER + " TEXT, " +
    	MESSAGE_ORDER_DETAIL + " TEXT, " +
    	MESSAGE_COMMENT_SHOP + " TEXT, " +
    	MESSAGE_PARAMS + " TEXT, " +
    	MESSAGE_TIME+ " TEXT, " +
    	MESSAGE_TITLE + " TEXT);";
    private static final String[] MESSAGE_TABLE_CREATE2 = {MESSAGE_USERID,MESSAGE_ID,MESSAGE_TYPE,MESSAGE_CONTENT,MESSAGE_ACTION_TYPE,
    	MESSAGE_SEARCH_SHOP,MESSAGE_SERVICE_DETAIL,MESSAGE_CANCEL_ORDER,MESSAGE_ORDER_DETAIL,MESSAGE_COMMENT_SHOP,MESSAGE_PARAMS,MESSAGE_TIME,MESSAGE_TITLE};
    ...
    private static final String[] ALARM_TABLE_CREATE2 = {ALARM_ID,ALARM_USERID,ALARM_FAULTCODE,ALARM_NAME,ALARM_CONTENT,
    	                                                 ALARM_TYPE,ALARM_VEHICLE_VIN,ALARM_OBDSN,ALARM_REPORT_TIME};
...
//		faultVerHelper = new FaultVerDbHelper(currenCont,DIC_VER_TABLE_NAME,null,DIC_DATABASE_VERSION);
//		faultHelper = new FaultDbHelper(currenCont,DICTIONARY_TABLE_NAME,null,DATABASE_VERSION);
//		messageHelper = new MessageDbHelper(currenCont,MESSAGE_TABLE_NAME,null,DATABASE_MES_VERSION);
//		alarmHelper = new AlarmDbHelper(currenCont,ALARM_TABLE_NAME,null,DATABASE_ALARM_VERSION);
	}
	...
	public ArrayList<FaultCodeInfo> getAllMyFaultCodes(String type){
		...
		if(db!=null&&db.isOpen()){
			...
			Cursor cr = db.query(DICTIONARY_TABLE_NAME, DICTIONARY_TABLE_CREATE2,FAULT_TYPT+"='common'", null, null, null, null); 
			if(cr!=null){
				if(!cr.moveToFirst()){
					cr.close();
					return null;
	...
	public ArrayList<FaultCodeInfo> getSomeFaultCodesById(String ModleId,String code){
		...
		if(db!=null&&db.isOpen()){
			...
			Cursor cr = db.query(DICTIONARY_TABLE_NAME, DICTIONARY_TABLE_CREATE2,FAULT_TYPT+"='"+ModleId+"' AND "+FAULT_CODE+"='"+code+"'"/*FAULT_CODE+"='"+code+"'"*/, null, null, null, null); 
			if(cr!=null){
				if(!cr.moveToFirst()){
					cr.close();
					return null;
	...
	public String getFaultCodesVersionByModle(String cModleId){
		...
		if(db!=null&&db.isOpen()){
			...
			Cursor cr = db.query(DIC_VER_TABLE_NAME, DIC_VER_TABLE_CREATE2,FAULT_DIC_MODLE+"='"+cModleId+"'", null, null, null, null); 
			if(cr!=null){
				if(!cr.moveToFirst()){
					cr.close();
					return null;
	...
	public ArrayList<TonggouMessage> getAllMyMessages(String userid){
		...
		try{
			if(db!=null&&db.isOpen()){
				...
				//Cursor cr = db.query(MESSAGE_TABLE_NAME, MESSAGE_TABLE_CREATE2,MESSAGE_USERID+"='"+userid+"'", null, null, null, null); 
				Cursor cr = db.query(MESSAGE_TABLE_NAME, MESSAGE_TABLE_CREATE2,MESSAGE_USERID+"='"+userid+"'", null, null, null, MESSAGE_TIME+" desc"); 
				if(cr!=null){
					if(!cr.moveToFirst()){
						cr.close();
						return null;
					...
					do{
						...
						String params= cr.getString(10);         //：actionType所依赖的业务数据      String
	...
	public void deleteOneMessage(String userid ,String mesId){
		...
		try{
			db = myDbHelper.getWritableDatabase();
			if(db!=null&&db.isOpen()){
				db.delete(MESSAGE_TABLE_NAME, MESSAGE_USERID+"='"+userid+"' AND "+MESSAGE_ID+"='"+mesId+"'", null);
			}
		}catch(Exception e){
			e.printStackTrace();
		}	
	...
	public void upDateOneMessage(String userid ,String mesId){
		...
		try{
			...
			if(db!=null&&db.isOpen()){
				...
				db.update(MESSAGE_TABLE_NAME, values, MESSAGE_USERID+"='"+userid+"' AND "+MESSAGE_ID+"='"+mesId+"'", null);
	...
	public ArrayList<String> getEarlyMessage(String userID,String limit){
		...
		if(db!=null&&db.isOpen()){
			...
			Cursor cr = db.query(MESSAGE_TABLE_NAME, MESSAGE_TABLE_CREATE2,MESSAGE_USERID+"='"+userID+"'", null, null, null, MESSAGE_TIME +" DESC",limit); 
			if(cr!=null){
				if(!cr.moveToFirst()){
					cr.close();
					return all;
	...
	public boolean saveAlarm(CarCondition condition){		
		...
		if(result == 1){  //新建
			...
			try{
				...
				db.update(ALARM_TABLE_NAME, values, ALARM_USERID+"='"+condition.getUserID()+"' AND "+ALARM_FAULTCODE+"='"+condition.getFaultCode()+"'", null);	
	...
	private int findAlarm(String usid,String faultCode){
		...
		if(db!=null&&db.isOpen()){
			try{
				Cursor cr = db.query(ALARM_TABLE_NAME, ALARM_TABLE_CREATE2,ALARM_USERID+"='"+usid+"' AND "+ALARM_FAULTCODE+"='"+faultCode+"'", null, null, null, null); 
				if(cr!=null){
					if(!cr.moveToFirst()){
						cr.close();
						return 1;
	...
	public void readedOneAlarm(String uid,String faultCode){
		...
		try{
			...
			db.update(ALARM_TABLE_NAME, values, ALARM_USERID+"='"+uid+"' AND "+ALARM_FAULTCODE+"='"+faultCode+"'", null);	
	...
	public void deleteOneAlarm(String userid ,String faultCode){
		...
		try{
			db = myDbHelper.getWritableDatabase();
			if(db!=null&&db.isOpen()){
				db.delete(ALARM_TABLE_NAME, ALARM_USERID+"='"+userid+"' AND "+ALARM_FAULTCODE+"='"+faultCode+"'", null);
			}
		}catch(Exception e){
			e.printStackTrace();
		}	
	...
	public ArrayList<CarCondition> getAllCarConditons(String userId){
		...
		if(db!=null&&db.isOpen()){
			// 最迟的在最前面
			Cursor cr = db.query(ALARM_TABLE_NAME, ALARM_TABLE_CREATE2,ALARM_USERID+"='"+userId+"'", null, null, null, ALARM_ID + " DESC"); 
			if(cr!=null){
				if(!cr.moveToFirst()){
					cr.close();
					return all;
				...
				do{
					CarCondition yf = new CarCondition();
					    ...
					    String tempFAULTCODE = cr.getString(2);       			//故障码 如果有多个故障码请以逗号 ,分开
	...
	public ArrayList<CarCondition> getAllUnReadCarConditons(String userId){
		...
		try{
			if(db!=null&&db.isOpen()){
				...
				Cursor cr = db.query(ALARM_TABLE_NAME, ALARM_TABLE_CREATE2,ALARM_USERID+"='"+userId+"' AND "+ALARM_NAME+"='UNREAD'", null, null, null, null); 
				if(cr!=null){
					if(!cr.moveToFirst()){
						cr.close();
						return all;
...
//	    public FaultVerDbHelper(Context context, String name,CursorFactory factory, int version) {
//			super(context, name, factory, version);
//		}
//	
//	    @Override
...
//	    public FaultDbHelper(Context context, String name,CursorFactory factory, int version) {
//			super(context, name, factory, version);
//		}
//	
//	    @Override
...
//	    public MessageDbHelper(Context context, String name,CursorFactory factory, int version) {
//			super(context, name, factory, version);
//		}
//	
//	    @Override
...
//	    public AlarmDbHelper(Context context, String name,CursorFactory factory, int version) {
//			super(context, name, factory, version);
//		}
//	
//	    @Override
