...
public class LoginActivity extends BaseActivity {
	...
	public void onForgetPwdButtonClick(View view) {
		...
		String accountNo = mUserNameEdit.getText().toString();
		searchpasswordEdit.setText( accountNo );
		Button searchpassword=(Button) window.findViewById(R.id.searchpasswordok);
		...
		request.doRequest(this, new AsyncJsonBaseResponseParseHandler<LoginResponse>() {
			...
			public void onParseSuccess(LoginResponse result, byte[] originResult) {
				...
				TongGouApplication.getInstance()
					.saveSomeInformation(LoginActivity.this, result,sharedPreferences, username, password);
				goToHomePage();
			}
	...
	private void doSearchPwd(final String userName) {
		...
		client.get( findPwdUrl , null, new AsyncJsonBaseResponseParseHandler<BaseResponse>() {
	...
	public void onGuestModeButtonClick(View view) {
		...
		if( isLoginExpress || isLogout || !MainActivity.ifAutoLogin) {
			goToHomePage();
		} else {
			// 回到原来界面
			finish();
