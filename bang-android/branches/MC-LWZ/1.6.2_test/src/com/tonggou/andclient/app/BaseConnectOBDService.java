...
//		devices = new ArrayList<BluetoothDevice>();
		...
		for (int i = 0; i < TongGouApplication.obdLists.size(); i++) {
			if ("YES".equals(TongGouApplication.obdLists.get(i).getIsDefault())) {
				currentDefaultObdSn = TongGouApplication.obdLists.get(i).getObdSN(); // 找到服务器送下来的默认obd
				break;
			}
		}
...
//			}
	...
	private void doSendReadRTDOrder() {
		TongGouApplication.showLog("send ReadRTDOrder");
		if( myBTService == null ) {
			return;
		}
		new Thread(){
...
//				myBTService.write("##DTC\r\n".getBytes());
	...
	private void doSentReadDTCOrder() {
		TongGouApplication.showLog("onSentReadDTCOrder");
		if( myBTService == null ) {
			return;
		}
		new Thread(){
...
//	                	}
	                	...
	                	TongGouApplication.showLog("device2:"+device.getName()+":"+device.getAddress());
//					}
	            ...
	            // When discovery is finished, change the Activity title
	            	...
	            	TongGouApplication.showLog("DISCOVERY_FINISHED");
	            }
	        }
	    };
