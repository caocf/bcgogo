...
//			ClientConnectionManager clientManager = new ThreadSafeClientConnManager(params, schemeRegistry);
			HttpRequestClient client = new HttpRequestClient(appContext);
			defaultHttpClient = (DefaultHttpClient) client.getDefaultClient().getHttpClient();
...
//		}
	...
	public NetworkState httpGetUpdateString(String uri, JSONParseInterface jsonParser){
		...
		TongGouApplication.showLog("httpGet  " + uri);
...
//			}
			...
			if(str!=null&&str.indexOf("登录过期")!=-1&&str.indexOf("-202")!=-1){	
				...
				jsonParser.parsing(str);    //解析
				TongGouApplication.showLog( "httpGet  - " + str);
				netState.setNetworkState(true);
				return netState;
	...
	private void printHeader(Header[] headers) {
		if( headers == null ) {
			TongGouApplication.showLog( "header empty" );
			return;
		}
		TongGouApplication.showLog("header -- print");
		for( Header header : headers) {
			TongGouApplication.showLog( header.getName() + "   " + header.getValue() );
		}
	}
...
//		}
		TongGouApplication.showLog("httpPost  " + uri);
		int stateCode;		
		HttpPost httpPost = new HttpPost(uri);		
//		if(JSESSIONIDStr!=null){
//			httpPost.setHeader("Cookie", "JSESSIONID=" + JSESSIONIDStr); 
//		}
		printHeader(httpPost.getAllHeaders());
		try {
...
//			}
			...
			jsonParser.parsing(str);    //解析
			TongGouApplication.showLog("put request result --> " + str );
			netState.setNetworkState(true);
...
//		}
		...
		TongGouApplication.showLog("httpPut  " + uri);
		TongGouApplication.showLog("httpPut params " + new String(valuePairs));
//		if(JSESSIONIDStr!=null){
...
//			}
			...
			TongGouApplication.showLog("httpPut result" + str);
			//Log.d("NETT", str);
			jsonParser.parsing(str);    //解析
			netState.setNetworkState(true);
...
//		}
		int stateCode;		
		TongGouApplication.showLog("httpDelete  " + uri);
...
//		}
		...
		printHeader(httpDelete.getAllHeaders());
		try {						
...
//			}
			...
			TongGouApplication.showLog("httpDelete result" + str);
			jsonParser.parsing(str);    //解析
			netState.setNetworkState(true);
			return netState;
...
//		TongGouApplication.showLog( httpClient.getCookieStore().toString() );
//		for (int i = 0; i < cookies.size(); i++) {
//			Cookie cookie = cookies.get(i);
//			String cookieName = cookie.getName();
//			String cookieValue = cookie.getValue();
//			TongGouApplication.showLog("cookieName  " + cookieName + "   cookieValue  " + cookieValue);
