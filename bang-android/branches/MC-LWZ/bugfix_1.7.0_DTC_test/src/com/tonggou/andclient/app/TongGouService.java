...
import com.tonggou.andclient.RemindEvaluateActivity;
...
//		 		}
	...
	private void getNotification(MessageType...types ){
		...
		request.doRequest(this, new AsyncJsonBaseResponseParseHandler<PollingMessagesResponse>() {
			...
			@Override
			public void onParseSuccess(PollingMessagesResponse result, byte[] originResult) {
	...
	private void handlerPollingMessage(String username, List<TonggouMessage> messages) {
		...
		if(messages!=null&&messages.size()>0){
			...
			for(int i=0;i<messages.size();i++){
				...
				if("COMMENT_SHOP".equals(messages.get(i).getActionType())){ 
					mesString = messages.get(i).getParams();
					Intent newIntent = new Intent(TongGouService.this, RemindEvaluateActivity.class); 
					newIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);  //注意，必须添加这个标记，否则启动会失败 
					newIntent.putExtra("message", mesString);
