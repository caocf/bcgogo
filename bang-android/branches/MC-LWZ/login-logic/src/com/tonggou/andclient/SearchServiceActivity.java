...
//					Toast.makeText(SearchServiceActivity.this,getString(R.string.nextpage_nodata),Toast.LENGTH_LONG).show();
	...
	private void doGetServicesByPage() {
		...
		new Thread(){
			public void run(){
				getServices("NULL","unfinished",currentUsername,mPager.getCurrentPage() + 1,INFO.ITEMS_PER_PAGE+"");
			}
		}.start();
	}
...
//		}
	...
	private void getNextPage() {
		...
		new Thread() {
			...
			public void run() {
//				pageNumNo++;
				if (!nextPageNetworking("NULL","unfinished",currentUsername,mPager.getCurrentPage() + 1,INFO.ITEMS_PER_PAGE+"")) {
//					pageNumNo--;
