...
public abstract class AbsVehicleManagerActivity extends DateTimePickerBackableActivity {
	...
	protected void onTitleBarCreated(SimpleTitleBar titleBar) {
		...
		titleBar.setRightButton(getString(R.string.btn_save), Color.TRANSPARENT)
	...
	protected void performTitlebarRightBtnClick() {
		if( !invidateInput() ) {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if( resultCode == RESULT_OK  ) {
			if( requestCode == ViolationAreaSelectActivity.REQUEST_CODE_SELECT_CITY ) {
				...
				String vehicleNo = data.getStringExtra(EditVehicleNoActivity.EXTRA_RESULT_DATA_VEHICLE_NO);
				if( ! getText(mVehicleModelIndicatorText.getTextView()).equalsIgnoreCase(vehicleNo) ) {
					mVehicleNoIndicatorText.setTextValue(vehicleNo);
					onVehicleNoChanged();
				}
			}
		}
