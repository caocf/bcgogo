	...
	private void updateMyDic(){
		...
		if (TongGouApplication.sObdLists != null && TongGouApplication.sObdLists.size() > 0) {
			for (int i = 0; i < TongGouApplication.sObdLists.size(); i++) {
				VehicleInfo cuVehicleInfo = TongGouApplication.sObdLists.get(i).getVehicleInfo();
	   	 	...
	   	 	MainActivity.this.runOnUiThread(new Runnable(){
				  public void run() {
					  ...
					  if(TongGouApplication.connetedOBD){
						  ...
						  carStatusIcon.setImageResource(R.drawable.car1);
						  if(!TextUtils.isEmpty(defaultBrandAndModle)){
							  if( defaultBrandAndModle.contains("null") ) {
								  carName.setText("未知品牌车型");
							  } else {
								  carName.setText(defaultBrandAndModle);
							  }
						  }else{
...
//						  }
    ...
    private void checkErrorAlert(){ 
    	...
    	ArrayList<CarCondition> havealarms = SaveDB.getSaveDB(this).getAllCarConditons(currentUsername, vehicleId);		
	...
	public void onConditionClickListener(View view) {
		...
		turnToActivity(MyVehicleConditionActivity.class, R.string.car_conditon_title);
