...
public class TongGouApplication extends Application {
	...
	private void checkCommonFaultDicDB() {
		SharedPreferences sp = getSharedPreferences(BaseActivity.SETTING_INFOS, Context.MODE_PRIVATE);
	...
	private boolean copyCommonFaultDicDB() {
		...
		try {
			...
			outputStream = new FileOutputStream(getDatabasePath(BaseActivity.COMMON_FAULT_DIC_FILE_NAME));
			int len = -1;
	...
	private String getCommonFaultDicVersion() {
		...
		try {
			if (mCommonFaultDicDB.isOpen()) {
				Cursor query = mCommonFaultDicDB.query(SaveDB.DIC_VER_TABLE_NAME, SaveDB.DIC_VER_TABLE_CREATE2,
	...
	private List<FaultCodeInfo> getFaultCodeList() {
		...
		if (mCommonFaultDicDB.isOpen()) {
			Cursor query = mCommonFaultDicDB.query(SaveDB.DICTIONARY_TABLE_NAME, null, null, null, null,
					null, null);
	...
	public class MyLocationListener implements BDLocationListener {
		...
		public void onReceiveLocation(BDLocation location) {
			...
			if (location.getLocType() != 61 && location.getLocType() != 65 && location.getLocType() != 66
			...
			if (baidubackStr != null && baidubackStr.indexOf("E") == -1) {
				...
				// TongGouApplication.showLog(location.getLatitude() + "  " +
				...
				if (location.getCityCode() == null || "".equals(location.getCityCode())
					...
					getSharedPreferences(BaseActivity.SETTING_INFOS, 0).edit()
							.putString(BaseActivity.LOCATION_LAST_CITYCODE, location.getCityCode()).commit();
				}
	...
	public void saveSomeInformation(Context context, LoginResponse loginResponse,
		...
		if (!TextUtils.isEmpty(userPassword)) {
			PreferenceUtil
					.putString(context, BaseActivity.SETTING_INFOS, BaseActivity.PASSWORD, userPassword);
		}
		...
		if (appConfig != null) {
			...
			if (obdReadInterval != null && !"".equals(obdReadInterval)) {
				sharedPreferences.edit().putString(BaseActivity.APPCONFIG_OBD_READ_INTERVAL, obdReadInterval)
						.commit();
			}
			String serverReadInterval = appConfig.getServerReadInterval();
			if (serverReadInterval != null && !"".equals(serverReadInterval)) {
				sharedPreferences.edit()
						.putString(BaseActivity.APPCONFIG_SERVER_READ_INTERVAL, serverReadInterval).commit();
			}
	...
	public void initDefaultVehicleList() {
	...
	public synchronized void setDefaultVehicleBindOBDs(List<VehicleInfo> data) {
		...
		String userNo = PreferenceUtil.getString(mInstance, BaseActivity.SETTING_INFOS, BaseActivity.NAME);
		...
		for (VehicleInfo vehicle : data) {
			if ("YES".equals(vehicle.getIsDefault())) {
				...
				if (!TextUtils.isEmpty(vehiNo)) {
					PreferenceUtil.putString(this, BaseActivity.SETTING_INFOS, BaseActivity.VEHICLENUM,
							vehiNo);
				}
				if (!TextUtils.isEmpty(vehiModelId)) {
					PreferenceUtil.putString(this, BaseActivity.SETTING_INFOS, BaseActivity.VEHICLE_MODE_ID,
							vehiModelId);
				}
		...
		if (TongGouApplication.connetedOBD && sObdLists.containsAll(newObdData)
				&& newObdData.containsAll(sObdLists)) {
			TongGouApplication
					.showLog("obdLists.containsAll(newObdData) && newObdData.containsAll(obdLists)");
		} else {
			sObdLists.clear();
	...
	public synchronized void queryVehicleList(Context context) {
		...
		final String userName = PreferenceUtil.getString(this, BaseActivity.SETTING_INFOS,
				BaseActivity.NAME);
		final boolean oilPriceSaved = !TextUtils.isEmpty(PreferenceUtil.getString(this,
				BaseActivity.SETTING_INFOS, BaseActivity.EXTRA_DEFAULT_OIL_PRICE));
		...
		request.doRequest(context, new AsyncJsonBaseResponseParseHandler<VehicleListResponse>() {
			...
			public void onParseSuccess(VehicleListResponse result, String originResult) {
				...
				if (!oilPriceSaved) {
					PreferenceUtil.putString(getApplicationContext(), BaseActivity.SETTING_INFOS,
							BaseActivity.EXTRA_DEFAUL_TOIL_KIND, result.getDefaultOilKind());
					PreferenceUtil.putString(getApplicationContext(), BaseActivity.SETTING_INFOS,
							BaseActivity.EXTRA_DEFAULT_OIL_PRICE, result.getDefaultOilPrice());
				}
	...
	public void deInit() {
		...
		getSharedPreferences(BaseActivity.SETTING_INFOS, 0)
				...
				.putString(BaseActivity.PHONENAME, null).putString(BaseActivity.SERVICE, null)
				.putString(BaseActivity.PHONE, null).putString(BaseActivity.BRAND, null)
				.putString(BaseActivity.MODEL, null).putString(BaseActivity.VEHICLENUM, null)
				.putString(BaseActivity.CHECKVOICE, null).putString(BaseActivity.VEHICLE_MODE_ID, "")
