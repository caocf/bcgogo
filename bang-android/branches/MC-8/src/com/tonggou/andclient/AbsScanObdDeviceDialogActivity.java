...
public abstract class AbsScanObdDeviceDialogActivity extends AbsUMengActivity implements OnItemClickListener, OnConnectCallback {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                if (mNewAdapter.getCount() == 0) {
                    String noDevices = getString(R.string.scan_bluetooth_no_new_device);
                    OBDDevice obdDevice = new OBDDevice();
                    obdDevice.setDeviceName(noDevices);
                    obdDevice.setDeviceAddress("");
                    mNewAdapter.add(obdDevice);
                }
	 ...
	 * 显示在加载的 progressbar
	 */
	private void showScanDevicesIndiactor() {
	...
	private void findDevices() {
		...
		if( pairedDevices.size() <= 0 ) {
			// 说明没有已配对设备
			String noDevices = getString(R.string.scan_bluetooth_no_paired_device);
            OBDDevice obdDevice = new OBDDevice();
            obdDevice.setDeviceName(noDevices);
            obdDevice.setDeviceAddress("");
	...
	class DeviceAdapter extends AbsViewHolderAdapter<OBDDevice> {
		...
		public void add(OBDDevice device) {
			List<OBDDevice> data = getData();
			if( data != null ) {
				data.add(device);
