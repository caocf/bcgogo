...
public class StoreVehicleInfoRequest extends AbsTonggouHttpRequest {
	...
	public void setRequestParams(String userNo, String vehicleId, String vehicleNo, String vehicleBrand, String vehicleModel, VehicleInfo vehicle, String bindingShopId) {
		...
		if( vehicle != null ) {
			...
			if( !TextUtils.isEmpty(nextInsuranceTime) && !"null".equalsIgnoreCase(nextInsuranceTime)) {
				params.put("nextInsuranceTime", Long.valueOf( SomeUtil.StringDateToLong( nextInsuranceTime) ));
			} 
			if( !TextUtils.isEmpty(nextExamineTime) && !"null".equalsIgnoreCase(nextExamineTime)) {
				params.put("nextExamineTime", Long.valueOf( SomeUtil.StringDateToLong(nextExamineTime) ));
			} 
		}
