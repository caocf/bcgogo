				...
				new AsyncJsonBaseResponseParseHandler<UpdateResponse>() {
					...
					public void onParseSuccess(UpdateResponse result,
						...
						if( action == UpdateAction.normal 
								...
								|| TextUtils.isEmpty(result.getDescription())
								|| !result.getUrl().trim().toLowerCase(Locale.getDefault()).startsWith("https"))
							 onNoraml();
						else
							 onAlert(result.getUrl(), result.getDescription(), action == UpdateAction.force);
					}
