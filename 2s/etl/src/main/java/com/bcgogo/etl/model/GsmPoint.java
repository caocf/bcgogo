...
public class GsmPoint extends LongIdentifier {
  ...
  public GsmPointDTO toDTO() {
    GsmPointDTO gsmPointDTO = new GsmPointDTO();
    gsmPointDTO.setId(getId());
    gsmPointDTO.setEmi(getEmi());
    gsmPointDTO.setAppUserNo(getAppUserNo());
    gsmPointDTO.setState(getState());
    gsmPointDTO.setAppPassword(getAppPassword());
    gsmPointDTO.setGroup(getGroup());
    gsmPointDTO.setCellPos(getCellPos());
    gsmPointDTO.setLon(getLon());
    gsmPointDTO.setLonDir(getLonDir());
    gsmPointDTO.setLat(getLat());
    gsmPointDTO.setLatDir(getLatDir());
    gsmPointDTO.setVelocity(getVelocity());
    gsmPointDTO.setHeading(getHeading());
    gsmPointDTO.setDate(getDate());
    gsmPointDTO.setTime(getTime());
    gsmPointDTO.setGsmVehicleStatus(getGsmVehicleStatus());
    gsmPointDTO.setOrgInfo(getOrgInfo());
    gsmPointDTO.setGroupId(getGroupId());
    gsmPointDTO.setUploadTime(getUploadTime());
    gsmPointDTO.setUploadServerTime(getUploadServerTime());
    gsmPointDTO.setUploadServerTimeStr(DateUtil.convertDateLongToDateString(DateUtil.DATE_STRING_FORMAT_ALL, getUploadServerTime()));
    gsmPointDTO.setGsmPointType(getGsmPointType());
    gsmPointDTO.setImpactStrength(getImpactStrength());
    gsmPointDTO.setGpsLat(NumberUtil.convertGPSLat(this.getLat()));
    gsmPointDTO.setGpsLon(NumberUtil.convertGPSLot(this.getLon()));
    return gsmPointDTO;
  }
