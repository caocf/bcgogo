...
//        if (p.hasOwnProperty("onFinish")) paramsDetailsList.onFinish = p["onFinish"];
    ...
    show:function(cpName) {
        ...
        GLOBAL.debug("searchcomplete function name==show:" + this.autocomplete.isVisible());
    ...
    moveFollow:function(param) {
        var foo = APP_BCGOGO.Module.searchcomplete, o = param.node;
        if (!param.isMoveOnly) {
            foo.clear();
        }
        ...
        var contentPosition = {
            left: GLOBAL.Display.getX(o),
            top: GLOBAL.Display.getY(o) + 22
        };
    ...
    scrollIntoView:function() {
        var foo = APP_BCGOGO.Module.searchcomplete;
        window.scrollTo(0, GLOBAL.Display.getY(foo._relInst) - $(foo._relInst).height() - $(foo._target).height());
    },
