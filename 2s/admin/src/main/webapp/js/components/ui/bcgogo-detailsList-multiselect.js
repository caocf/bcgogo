...
APP_BCGOGO.Module.detailsListMultiselect = {
    ...
    getPageSize:function() {
        var foo = APP_BCGOGO.Module.detailsListMultiselect;
    ...
    _getAoColumns:function(arr) {
        var retVal = [], foo = APP_BCGOGO.Module.detailsListMultiselect;
    ...
    _parseAaData:function(idata) {
        var self = APP_BCGOGO.Module.detailsListMultiselect, odata = [], odataBreaked = [], foo = self._aoColumns;
...
//        GLOBAL.debug(data);
        ...
        if( foo._cachedPageData.hasOwnProperty(foo.getPageIndex().toString()) === false ) {
            ...
            foo._countArr.push(data["currentCount"]||foo.getPageSize());
        }
    ...
    _getPageState:function(){
        var foo = APP_BCGOGO.Module.detailsListMultiselect, state = "";
        if(foo.getPageIndex() === 0) {
            state = foo._pageState.FIRST;
        }

        var avgCount = 0;
        $.each(foo._countArr, function(index, value){
            avgCount += value;
            if( index === foo.getPageIndex() )
                return false;
        });

        if(!( foo._totalCount !== -1 && foo._totalCount > avgCount )) {
...
//        }
    ...
    _onDbClickBefore:function(event) {
        var foo = APP_BCGOGO.Module.detailsListMultiselect, index = foo._getRowIndexFromNode(event.currentTarget);
        foo.onDbClick(event, index, foo._data[index]);
    },

    _onPageBefore:function(event){
        var foo = APP_BCGOGO.Module.detailsListMultiselect, fnName;
...
//        var foo = APP_BCGOGO.Module.detailsListMultiselect, indexList = [], dataList = [];
