...
Ext.define('Ext.view.customerMange.ShopAdAreaScope', {
    alias: 'widget.shopadareascope',
    ...
    listeners: {
        beforerender: function () {
            var me = this,
                readUrl = Ext.Bcgogo.ShopAdAreaScope.readUrl,
                params = Ext.Bcgogo.ShopAdAreaScope.params;
            if (!readUrl) {
                readUrl = "shopAd.do?method=getShopAdAreaScopeByShopId";
            }
...
//                    afterrender: me.expandChecked
    ...
    save: function () {
        ...
        Ext.Array.each(records, function (rec) {
            if (rec.get('type') == 'SECOND_CATEGORY') {
                adAreaId.push(rec.get('id'));
            }
        });

        me.drawShowTable(me.getNamesByExtTree(tree.getRootNode()));

        openTarget.setShopAdAreaIds(adAreaId);
        me.close();
    },
    ...
    drawShowTable: function (names) {
        ...
        table += '</table>';
        this.getOpenTarget().down('[name=selectedAdAreaDisplay]')
            .setHeight(25 * rows)
            .setValue(table);
    },
