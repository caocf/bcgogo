...
import com.bcgogo.api.controller.UserController;
...
//      configService.setConfig("SmsSenderStrategy", SmsConstant.SmsYiMeiConstant.name, ShopConstant.BC_SHOP_ID);
  ...
  public RegistrationDTO  createAppUserAndLogin(){
    RegistrationDTO registrationDTO = new RegistrationDTO();
    registrationDTO.setUserNo("苏E552UQ");
    registrationDTO.setPassword("111111");
    registrationDTO.setMobile("18001557667");
    LoginDTO loginDTO = new LoginDTO();
    loginDTO.setAppVersion("1.0");
    loginDTO.setImageVersion("640X1136");
    loginDTO.setPlatform(AppPlatform.IOS);
    loginDTO.setMobileModel("iPhone 5 (GSM+CDMA)");
    loginDTO.setPlatformVersion("7.0.3");
    registrationDTO.setLoginInfo(loginDTO);
    UserController userController = new UserController();
    if(response == null){
      response = new MockHttpServletResponse();
    }
    ApiResponse apiResponse = userController.registration(response, registrationDTO);
