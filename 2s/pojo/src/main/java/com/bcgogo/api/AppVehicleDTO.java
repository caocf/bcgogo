...
public class AppVehicleDTO implements Serializable {
  ...
  public VehicleDTO toVehicleDTO() {
    ...
    dto.setLicenceNo(getVehicleNo());
    dto.setModel(getVehicleModel());
    dto.setBrand(getVehicleBrand());
    dto.setStartMileage(getCurrentMileage());
    if (getNextMaintainMileage() != null)
      dto.setMaintainMileage(getNextMaintainMileage().longValue());
    dto.setMaintainTime(getNextMaintainTime());
    dto.setInsureTime(getNextInsuranceTime());
    dto.setExamineTime(getNextExamineTime());
    dto.setStatus(VehicleStatus.ENABLED);
    return dto;
  }


  public String saveVehicleValidate() {
  ...
  public void from(ShopDTO shopDTO) {
    if (shopDTO != null) {
      setRecommendShopId(shopDTO.getId());
      setRecommendShopName(shopDTO.getName());
    }
  }

  public void fromVehicleDTO(VehicleDTO vehicleDTO) {
    if(StringUtils.isBlank(this.getVehicleNo())){
      this.setVehicleNo(vehicleDTO.getLicenceNo());
    }
    this.setVehicleBrand(vehicleDTO.getBrand());
    this.setVehicleBrandId(vehicleDTO.getBrandId());
    this.setVehicleModel(vehicleDTO.getModel());
    this.setVehicleModelId(vehicleDTO.getModelId());
    this.setNextMaintainMileage(NumberUtil.doubleVal(vehicleDTO.getMaintainMileage()));
    this.setNextInsuranceTime(vehicleDTO.getInsureTime());
  }

  public String saveVehicleConditionValidate() {
    if (StringUtil.isEmpty(userNo)) {
      return ValidateMsg.APP_USER_NO_EMPTY.getValue();
    }
//    if (StringUtil.isEmpty(vehicleVin)) {
//      return ValidateMsg.VEHICLE_VIN_EMPTY.getValue();
//    }
    if (vehicleId == null) {
  ...
  public void from(ObdDTO obdDTO) {
    if (obdDTO != null) {
      this.setObdSN(obdDTO.getSn());
    }
  }
