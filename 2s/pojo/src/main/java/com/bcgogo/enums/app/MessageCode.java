...
public enum MessageCode {
  ...
  LOGIN_USER_RE_LOGIN(-207, "用户账户已退出，请重新登录!", Status.FAIL),
  SESSION_ID_EMPTY(-208, "登录sessionId不能为空!", Status.FAIL),
  ...
  PASSWORD_NO_EMPTY(-302, "密码不能为空！", Status.FAIL),
  PASSWORD_IS_WRONG(-303, "密码不正确！", Status.FAIL),
  ...
  USER_APPOINT_SERVICE_FAIL_TWO(-4002, "{0}，用户预约服务失败！", Status.FAIL),
  ...
  UPLOAD_PARAM_EMPTY(8207, "上传参数不能为空！", Status.FAIL),
  ...
  VIDEO_UPLOAD_OVER_THE_MOUTH_LIMIT(8211, "您本月上传数量超过系统限制，上传失败！", Status.FAIL),
  IMAGE_SIZE_EMPTY(8212, "上传图片文件为空！", Status.FAIL),
  IMPACT_CONDITION_NOT_ENOUGH(8213, "碰撞条件不足！", Status.FAIL),
  ...
  //商城消费记录
  APP_MALL_SUCCESS(10000,"消费成功,账单已生成",Status.SUCCESS),
  APP_MALL_FAIL(10001,"商城消费失败",Status.FAIL),
  APP_MALL_COUPON_FAIL(10002,"代金券余额不足",Status.FAIL),
  APP_MALL_EXCEPTION(-10000,"商城消费异常",Status.FAIL),

  //上传GPS信息，获取店铺
  APP_SHOP_LIST_SUCCESS(11000,"获取店铺列表成功",Status.SUCCESS),
  APP_SHOP_GPS_FALL(11001,"GPS信息不能为空",Status.FAIL),
  APP_SHOP_EXCEPTION(-11000,"获取店铺列表异常",Status.FAIL),

  //远程实时监控拍照命令
  APP_MONITOR_SUCCESS(12000," 拍照请求成功",Status.SUCCESS),
  App_MONITOR_FAIL(12001,"正在上传，请耐心等候！",Status.FAIL),
  APP_MONITOR_EXCEPTION(-12000,"拍照请求异常",Status.FAIL),

  //获取代金券
  APP_COUPON_SUCCESS(13000,"获取代金券信息成功", Status.SUCCESS),
  APP_COUPON_FAIL(13001,"获取代金券失败",Status.FAIL),
  APP_COUPON_EXCEPTION(-13000,"获取代金券失败",Status.FAIL),

  //获取流量包
  APP_TRAFFIC_SUCCESS(14000,"获取在线商品成功",Status.SUCCESS),
  APP_TRAFFIC_FAIL(14001,"获取在线商品失败",Status.FAIL),

  //获取消费列表
  APP_CONSUMING_SUCCESS(15000,"获取消费信息列表成功",Status.SUCCESS),
  APP_CONSUMING_FAIL(15001,"获取消费信息列表失败",Status.FAIL),
  APP_CONSUMING_PARAM_FAIL(15002,"请求消费信息列表参数异常",Status.FAIL),
  APP_CONSUMING_EXCEPTION(-15000,"获取消费信息列表异常",Status.FAIL),
  APP_CONSUMING_DETAILS_SUCCESS(15100,"获取消费详情成功",Status.SUCCESS),
  APP_CONSUMING_DETAILS_FAIL(15101,"获取消费信息列表失败",Status.FAIL),
  APP_CONSUMING_DETAILS_PARAM_FAIL(15102,"请求消费详情参数异常",Status.FAIL),
  APP_CONSUMING_DETAILS_EXCEPTION(-15100,"获取消费详情异常",Status.FAIL),

  //APP推荐人
  APP_GET_RECOMMEND_SUCCESS(16200,"获取推荐人成功",Status.SUCCESS),
  APP_GET_RECOMMEND_FAIL(16201,"获取推荐人失败",Status.FAIL),
  APP_GET_RECOMMEND_EXCEPTION(-16200,"获取推荐人异常",Status.FAIL),
  APP_RECOMMEND_SUCCESS(16000 ,"填写成功，您和您的推荐人都将获得200元代金券奖励" ,Status.SUCCESS),
  APP_RECOMMEND_FAIL(16001 , "已获得赠送代金券" , Status.FAIL),
  APP_PHONE_RECOMMEND_FAIL(16002 , "抱歉，该推荐人不存在" , Status.FAIL),
  APP_RECOMMEND_EXCEPTION(-16000 ,"推荐异常" , Status.FAIL),
  APP_SHARED_SUCCESS(16100 ,"首次分享成功 ,您将获得200元代金券 " , Status.SUCCESS),
  APP_SHARED_FAIL(16101,"您已分享过",Status.FAIL),
  APP_SHARED_EXCEPTION(-16100 , "分享异常" , Status.FAIL),

  //APP支付
  WX_PREPARE_ORDER_SUCCESS(17000,"生成预支付订单成功",Status.SUCCESS),
  WX_PREPARE_ORDER_FAIL(17001,"生成预付订单失败",Status.FAIL),
  WX_PREPARE_ORDER_EXCEPTION(-17000,"生成预付订单参数异常",Status.FAIL),
