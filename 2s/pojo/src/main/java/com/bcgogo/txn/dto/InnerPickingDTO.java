 ...
 */
public class InnerPickingDTO extends BcgogoOrderDto {
  ...
  private InnerPickingItemDTO[] itemDTOs;
  ...
  public InnerPickingItemDTO[] getItemDTOs() {
    return itemDTOs;
  }

  public void setItemDTOs(InnerPickingItemDTO[] itemDTOs) {
  ...
  public OrderIndexDTO toOrderIndexDTO() {
    ...
    orderIndexDTO.setOrderType(OrderTypes.INNER_PICKING);
    ...
    if (!ArrayUtils.isEmpty(this.getItemDTOs())) {
      for (InnerPickingItemDTO innerPickingItemDTO : this.getItemDTOs()) {
        if (innerPickingItemDTO == null) continue;
        innerPickingItemDTO.setItemCostPrice(NumberUtil.doubleVal(innerPickingItemDTO.getInventoryAveragePrice())*innerPickingItemDTO.getAmount());
        //添加每个单据的产品信息
        inOutRecordDTOList.addAll(innerPickingItemDTO.toInOutRecordDTO(this));
      }
