<%--
    todo :this version should be merge with pageAjax.jsp
--%>
...
$(function () {
    ...
    $("#prePage" + dynamical).live('click', function () {
        var totalRows = null;
        if (dynamical == "StockSearchWithDownLimit") {
            totalRows = Number($("#totalRowsLowerLimit").val());
        } else if (dynamical == "StockSearchWithUpLimit") {
            totalRows = Number($("#totalRowsUpperLimit").val());
        } else {
    ...
    $("#getPage" + dynamical).live('keypress', function (e) {
        ...
        if (k == 13 || k == 108) {
            ...
            var pageRows = Number(data.pageRows);
            var totalPage = Math.ceil(totalRows / pageRows);
            var num = Number($("#getPage" + dynamical).val());
            if (isNaN(num)) {
                $("#getPage" + dynamical).val(currentPage);
                return;
            }
            ...
            var currentPage = num;
            data = {totalRows:totalRows,pageRows:pageRows,totalPage:totalPage,currentPage:currentPage};
            data.rowStart = data.pageRows * (num - 1);
            $("#rowStart").val(data.rowStart);
            var dataString = JSON.stringify(data);
            $("#data" + dynamical).val(dataString);
            pagesAjaxForSolrPost(formId, buttonId);
...
function initData(dynamicalID) {
    $("#rowStart").val(0);
    var data = {rowStart:0,pageRows:$("#pageRows").val(),currentPage:1};
    var dataString = JSON.stringify(data);
...
function initData(dynamicalID,currentPage) {
    ...
    $("#rowStart").val(0);
    var data = {rowStart:0,pageRows:$("#pageRows").val(),currentPage:currentPage};
    var dataString = JSON.stringify(data);
    $("#data" + dynamicalID).val(dataString);
}
...
function initPagesForSolr(dynamicalID, formId, buttonId) {
    var totalRows = null;
    if (dynamicalID == "StockSearchWithDownLimit") {
        totalRows = Number($("#totalRowsLowerLimit").val());
    } else if (dynamicalID == "StockSearchWithUpLimit") {
        totalRows = Number($("#totalRowsUpperLimit").val());
    } else {
        totalRows = Number($("#totalRows").val());
    }
...
<div class="i_pageBtn i_solrPageBtn" id="i_pageBtn<%=request.getParameter("dynamical")%>" style="display: none">
    ...
    <div class="pageNum" id="totalRowS<%=request.getParameter("dynamical")%>">
        ...
        页，到第
        <input type="text" class="selectPage selectSolrPage"
