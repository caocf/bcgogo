   ...
   <script type="text/javascript" src="js/components/ui/bcgogo-highlightcomplete<%=ConfigController.getBuildVersion()%>.js"></script>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
                    ...
                    <%if(request.getParameter("isEditBusinessAccount")==null)
                   { %>
  ...
  <form:form commandName="businessAccountDTO" id="businessAccountForm"
    ...
    <div class="divTit">
      ...
      <div style="display:inline-block;">
        ...
        <c:if test="${businessAccountDTO.moneyCategory=='income'}">
          <input id="moneyCategoryIncome" name="moneyCategoryRadio" style="margin-top: 3px;" type="radio" checked="checked" value="income"/> 收入
          <input id="moneyCategoryExpenses" name="moneyCategoryRadio" style="margin-top: 3px;" type="radio" value="expenses"/> 支出
        </c:if>

        <c:if test="${businessAccountDTO.moneyCategory=='expenses'}">
          <input id="moneyCategoryIncome" name="moneyCategoryRadio" type="radio" value="income"/> 收入
          <input id="moneyCategoryExpenses" name="moneyCategoryRadio" type="radio" checked="checked" value="expenses"/> 支出
        </c:if>
        <form:input path="moneyCategory" type="hidden" value="${businessAccountDTO.moneyCategory}"/>
    ...
    <div class="divTit" style="margin-right:5px;">
      ...
      <form:input path="accountCategory" class="txt" style="width:160px;color: #444443;" maxlength="5"
                  value="${businessAccountDTO.accountCategory}"/>
    </div>
    <div class="divTit">
    ...
    <div class="divTit" style="margin-right:50px;">
      ...
      <c:choose>
        <c:when test="${businessAccountDTO.departmentId!=null}">
          <form:select path="departmentId" cssStyle="width:165px;color: #444443;">
            <option value="${businessAccountDTO.departmentId}">${businessAccountDTO.dept}</option>
            <option value="">请选择</option>
            <c:forEach items="${departmentDTOList}" var="item">
              <c:if test="${item.idStr!=businessAccountDTO.departmentId}">
    ...
    <div class="divTit" style="margin-right:20px;">
      ...
      <c:choose>
        <c:when test="${businessAccountDTO.salesManId!=null}">
          <form:select path="salesManId" cssStyle="width:165px;color: #444443;">
            <option value="${businessAccountDTO.salesManId}">${businessAccountDTO.person}</option>
            <option id="personFirstSelect" value="">请选择</option>
            <c:forEach items="${salesManDTOList}" var="item">
              <c:if test="${item.idStr!=businessAccountDTO.salesManId}">
                <option value="${item.idStr}">${item.name}</option>
              </c:if>
            </c:forEach>
          </form:select>
    ...
    <form:input path="total" type="hidden" value="${businessAccountDTO.total==null?'0.0':businessAccountDTO.total}"
                readonly="true"/>
    ...
    <div class="button">
      ...
      <a id="saveBtn" class="btnSure">修 改</a>
