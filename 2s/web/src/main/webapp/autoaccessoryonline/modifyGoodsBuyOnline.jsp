...
<head>
    ...
    <script type="text/javascript">
        ...
        var orderSuccess = GLOBAL.Util.getUrlParameter("orderSuccess");
        $(document).ready(function () {
            <c:if test="${fn:length(purchaseOrderDTOList)==0}">
            nsDialog.jAlert("当前没有可以编辑的采购单,进入代办采购单进行查看!","友情提示",function(){
                window.location = "orderCenter.do?method=getTodoOrders&type=TODO_PURCHASE_ORDERS";
            });
            </c:if>
            if(!G.isEmpty(orderSuccess) && orderSuccess=='true'){
                nsDialog.jAlert("在线采购下单成功！请等待卖家处理订单！","友情提示");
            }
        });
    </script>
</head>
...
<c:forEach var="purchaseOrderDTO" items="${purchaseOrderDTOList}" varStatus="orderStatus">
<div class="J-OrderBlock J-OrderBlock-${purchaseOrderDTO.supplierShopId}">
<form:form commandName="purchaseOrderDTO" id="purchaseOrdersForm${orderStatus.index}" action="RFbuy.do?method=updatePurchaseOrderOnline" method="post">
...
<div class="cartBody">
    <div class="divTit" style="width:280px;"><span style="float:left;">供应商：</span>
        <a class="blue_color J-goSupplier ellipsis" data-supplierid="${purchaseOrderDTO.supplierId}" title="${purchaseOrderDTO.supplier}"
           style="margin:0;width:180px;">
            <b>${purchaseOrderDTO.supplier}</b></a>
    ...
    <div class="divTit" style="width:100px">联系人：<span>${empty purchaseOrderDTO.contact ? "--": purchaseOrderDTO.contact}</span> <input type="hidden" id="contact" name="contact" class="txt" maxlength="20" value="${purchaseOrderDTO.contact}" style="width:60px;"/></div>
    <div class="divTit" style="width:160px">联系方式：<span>${empty purchaseOrderDTO.mobile ? "--": purchaseOrderDTO.mobile}</span> <input type="hidden" id="mobile" name="mobile" class="txt" maxlength="11" value="${purchaseOrderDTO.mobile}" style="width:70px;"/></div>
    <div class="divTit" style="width:200px">所属区域：<span>${empty purchaseOrderDTO.areaInfo ? "--":purchaseOrderDTO.areaInfo}</span>
        ...
        <%--<select id="city" name="city"--%>
                <%--initValue="${purchaseOrderDTO.city}" class="txt select J_city" style="width: 80px">--%>
        <%--</select>--%>
        <%--<select id="region" name="region"--%>
                <%--initValue="${purchaseOrderDTO.region}" class="txt select J_region" style="width: 80px">--%>
        <%--</select>--%>
        <input type="hidden" id="address" name="address" class="txt J_address_input" maxlength="50" value="${purchaseOrderDTO.address}" style="width: 130px"/>
    </div>
                    ...
                    <c:choose>
                        <c:when test="${!purchaseOrderDTO.fromQuotedPreBuyOrder}">
                            <div style="float: left;">
                                <div>
                                    <c:choose>
                                        <c:when test="${not empty itemDTO.promotionsIds && itemDTO.price!=itemDTO.quotedPrice && !itemDTO.customPriceFlag}">
                                        <a class="J-oldPrice gray">${itemDTO.quotedPrice}&nbsp;
                                            <a class="J-newPrice yellow_color">${itemDTO.price}</a>
                                            <input type="hidden" autocomplete="off" class="purchasePrice" value="${itemDTO.quotedPrice}"/>
                                            <input type="hidden" class="bargainSaveValue" value="0"/>
                                        <span class="save J-SaveInfo">
        ...
        <tr>
            ...
            <td colspan="3">
                <bcgogo:hasPermission permissions="WEB.TXN.PURCHASE_MANAGE.PURCHASE.CANCEL">
                    <a class="btnCancle J-repealOrder">作废订单</a>
                </bcgogo:hasPermission>
                <bcgogo:hasPermission permissions="WEB.TXN.PURCHASE_MANAGE.PURCHASE.SAVE">
                    <a class="btnCancle J-modifyOrder" id="J-modifyOrder">修改订单</a>
                </bcgogo:hasPermission>
            </td>
        </tr>
    </table>
...
<c:if test="${fn:length(purchaseOrderDTOList)>1}">
    <div class="J-floatHeightDiv" style="clear: both;height: 45px"></div>
</c:if>
<div class="order J-floatDiv" style="${fn:length(purchaseOrderDTOList)>1?'position:fixed;bottom:0px':'position:static;bottom:0px'}">
    <c:choose>
        <c:when test="${returnLinkType=='CommodityQuotations'}">
            <a class="blue_color" href="autoAccessoryOnline.do?method=toCommodityQuotations" style="position:absolute;">返回配件报价</a>
        </c:when>
        <c:otherwise>
            <a class="blue_color" href="shoppingCart.do?method=shoppingCartManage" style="position:absolute;">返回购物车</a>
        </c:otherwise>
    </c:choose>
