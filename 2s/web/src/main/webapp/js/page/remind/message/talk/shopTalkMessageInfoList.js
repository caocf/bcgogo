...
//        });
    ...
    $("#deleteAction").click(function () {
        ...
        if (G.Lang.isEmpty(ids)) {
            nsDialog.jAlert("请选择需要删除的救援记录！");
        } else {
            nsDialog.jConfirm("您确定要删除所选救援记录吗？", null, function (resultVal) {
                if (resultVal) {
                    APP_BCGOGO.Net.asyncGet({
                        url: "shopSosInfo.do?method=deleteSosInfo",
                        data: {'id': ids},
                        dataType: "json",
                        success: function (result) {
                            searchShopFaultInfo();
...
//    if (timeStart)data['timeStart'] = new Date(Date.parse(timeStart.replace(/-/g, "/"))).getTime();
//    if (timeEnd)data['timeEnd'] = new Date(Date.parse((timeEnd + " 23:59").replace(/-/g, "/"))).getTime();
        var url = "shopTalkMessageInfo.do?method=searchShopTalkMessageInfoList";
        APP_BCGOGO.Net.asyncPost({
            url: url,
            data: data,
            dataType: "json",
...
function drawShopFaultInfoTable(json) {
    ...
    if (!G.isEmpty(json)) {
        ...
        var countGt2 = 0;
        if (json['todayShopTalkMessageInfoList'] && json['todayShopTalkMessageInfoList'].length > 0) {
            countGt2 ++;
        }
        ...
        drawTableArea(json['todayShopTalkMessageInfoList'], isShowTitle ? json['todayTotalRows'] : "", "今天");
    ...
    function drawTableArea(datas, numberStr, titleDayStr) {
        ...
        if (!G.isEmpty(datas) && !G.isEmpty(datas[0])) {
            ...
            $.each(datas, function (index, value) {
                var content = G.Lang.normalize(value['content']);
                var sendTime = G.Lang.normalize(value['sendTime']);
                ...
                var customer = G.Lang.normalize(value['customer']);
                var sendTimeStr = G.Lang.normalize(value['sendTimeStr']);
                var replyTimeStr = G.Lang.normalize(value['replyTimeStr']);
                var replyContent = G.Lang.normalize(value['replyContent']);
                var vehicleContact = G.Lang.normalize(value['vehicleContact']);
                var vehicleMobile = G.Lang.normalize(value['vehicleMobile']);
                var customerName = G.Lang.normalize(value['customerName']);
                var customerMobile = G.Lang.normalize(value['customerMobile']);
                var idStr = G.Lang.normalize(value['idStr']);
...
//        tr += '    <td class="score_table2_left"><input type="checkbox" data-id="' + idStr + '" data-vehicle="'+value['idStr']+'"/></td>';
                tr += '    <td class="score_table2_left">' + vehicleNo + '</td>';
                tr += '    <td>' + '<a class="blue_color" action="searchCustomerDetail" data-id="' + customerId + '">' + vehicleContact + ' ' + vehicleMobile + '</a>' + '</td>';
                tr += '    <td>' + '<a class="blue_color" action="searchCustomerDetail" data-id="' + customerId + '">' + customerName + ' ' + customerMobile + '</a>' + '</td>';
                tr += '    <td>' + sendTimeStr + '</td>';
                tr += '    <td>' + content + '</td>';
                tr += '    <td>' + replyTimeStr + '</td>';
                tr += '    <td>' + replyContent + '</td>';
...
//                tr += '    <td title="' + customerName + '">'+(customerName.length>11?customerName.substr(0,11)+'...':customerName)+'<br>'+customerMobile + '</td>';
                ...
                if (value['isCreateAppointOrder'] == 'YES') {
                    ...
                    tr += '<a class="blue_color" action="createAppointOrderByCustomerIdAndAppUserNo" data-id="' + appUserNo + '">回复</a>';
                }
...
$("[action=createAppointOrderByCustomerIdAndAppUserNo]").live("click", function() {
    var appUserNo = $(this).attr('data-id');
    if (!appUserNo) return;
    $("#talkDialog").dialog({
        resizable: true,
        title:"对话",
