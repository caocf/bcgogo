...
//        data['faultAlertType'] = $("[name=faultAlertType]").val();
        ...
        if (timeStart)data['timeStart'] = new Date(Date.parse(timeStart.replace(/-/g, "/"))).getTime();
        if (timeEnd)data['timeEnd'] = new Date(Date.parse((timeEnd + " 23:59").replace(/-/g, "/"))).getTime();
        var url = "shopImpactInfo.do?method=searchShopImpactInfoList";
        APP_BCGOGO.Net.asyncPost({
            url: url,
            data: data,
            dataType: "json",
...
function drawShopFaultInfoTable(json) {
    ...
    if (!G.isEmpty(json)) {
        ...
        var countGt2 = 0;
        if(json['todayShopImpactInfoList'] &&json['todayShopImpactInfoList'].length>0 ){
            countGt2 ++;
        }
        ...
        drawTableArea(json['todayShopImpactInfoList'], isShowTitle ? json['todayTotalRows'] : "","今天");
    ...
    function drawTableArea(datas, numberStr,titleDayStr) {
        ...
        if (!G.isEmpty(datas) && !G.isEmpty(datas[0])) {
            ...
            $.each(datas, function (index, value) {
                var impactVideoId = G.Lang.normalize(value['impactVideoIdStr']);
                var uploadTimeDateStr = G.Lang.normalize(value['uploadTimeDateStr']);
                var uploadTimeStr = G.Lang.normalize(value['uploadTimeStr']);
                var address = G.Lang.normalize(value['address']);
                var url = G.Lang.normalize(value['url']);
                ...
                var impactId = G.Lang.normalize(value['impactIdStr']);
                var url = G.Lang.normalize(value['url']);

                var lon =G.Lang.normalize(value['longitude']);
                var lat =G.Lang.normalize(value['latitude']);
                ...
                tr += '    <td class="score_table2_left"><input type="checkbox" data-id="' + impactVideoId + '" data-vehicle="'+value['impactVideoIdStr']+'"/></td>';
                tr += '    <td>' + uploadTimeDateStr + '</td>';
                tr += '    <td>' + address + '</td>';
                tr += '    <td>' + '<a class="blue_color" action="searchImpactVideoDetail" data-id="'+url+'">'+'查看视频' +'</a>'+ '</td>';
                tr += '    <td>' + '<a class="blue_color" action="searchCustomerDetail" data-id="'+customerId+'">'+vehicleNo+' '+vehicleBrand+' '+vehicleModel +'</a>'+ '</td>';
...
//                tr += '    <td title="' + customerName + '">'+(customerName.length>11?customerName.substr(0,11)+'...':customerName)+'<br>'+customerMobile + '</td>';
                ...
                if (value['isCreateAppointOrder'] == 'YES') {
                  ...
                  tr += ' ';
                  tr += '<a class="blue_color" action="impactDetail" data-id1="' + impactId + '" data-id2="' + uploadTimeStr + '">碰撞详情</a>';
                }
...
$("[action=searchImpactVideoDetail]").live("click",function () {
...
$("[action=impactDetail]").live("click", function() {
  var impactId = $(this).attr('data-id1');
  var uploadTime_=$(this).attr("data-id2");
  if(!impactId) return;
