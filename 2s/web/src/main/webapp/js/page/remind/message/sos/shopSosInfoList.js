...
//        data['mileageType'] = $("[name=mileageType]").val();
    ...
    if (timeStart)data['timeStart'] = new Date(Date.parse(timeStart.replace(/-/g, "/"))).getTime();
    if (timeEnd)data['timeEnd'] = new Date(Date.parse((timeEnd + " 23:59").replace(/-/g, "/"))).getTime();
    var url = "shopSosInfo.do?method=searchShopSosInfoList";
    APP_BCGOGO.Net.asyncPost({
      url: url,
      data: data,
      dataType: "json",
...
function drawShopFaultInfoTable(json) {
  ...
  if (!G.isEmpty(json)) {
    ...
    var countGt2 = 0;
    if(json['todayShopSosInfoList'] &&json['todayShopSosInfoList'].length>0 ){
      countGt2 ++;
    }
    ...
    drawTableArea(json['todayShopSosInfoList'], isShowTitle ? json['todayTotalRows'] : "","今天");
  ...
  function drawTableArea(datas, numberStr,titleDayStr) {
    ...
    if (!G.isEmpty(datas) && !G.isEmpty(datas[0])) {
      ...
      $.each(datas, function (index, value) {
        ...
        var uploadTime = G.Lang.normalize(value['uploadTimeStr']);
        ...
        var addr = G.Lang.normalize(value['addr']);
        var addrShort =  G.Lang.normalize(value['addrShort']);
        ...
        if(value['state'] == 'DELETED'){
          ...
          if (value['state'] == 'UNTREATED') {
            state += "未处理"
          }
          if (value['state'] == 'TREATED') {
            state += " 已处理"
          }
        }
        ...
        tr += '    <td>' + vehicleContact+' '+vehicleMobile + '</td>';
