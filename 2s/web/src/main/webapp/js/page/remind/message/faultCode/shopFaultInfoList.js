...
//        });
    ...
    $("#deleteAction").click(function () {
        ...
        if (G.Lang.isEmpty(ids)) {
            nsDialog.jAlert("请选择需要删除的故障记录！");
        } else {
            nsDialog.jConfirm("您确定要删除所选故障提醒吗？", null, function (resultVal) {
                if (resultVal) {
                    APP_BCGOGO.Net.asyncGet({
                        url: "shopFaultInfo.do?method=deleteFaultInfoCode",
                        data: {'id': ids},
                        dataType: "json",
                        success: function (result) {
                            searchShopFaultInfo();
    ...
    function searchShopFaultInfo() {
        ...
        if (timeStart)data['timeStart'] = new Date(Date.parse(timeStart.replace(/-/g, "/"))).getTime();
        if (timeEnd)data['timeEnd'] = new Date(Date.parse((timeEnd + " 23:59").replace(/-/g, "/"))).getTime();
        var url = "shopFaultInfo.do?method=searchShopFaultInfoList";
        APP_BCGOGO.Net.asyncPost({
            url: url,
            data: data,
            dataType: "json",
...
function drawShopFaultInfoTable(json) {
    ...
    if (!G.isEmpty(json)) {
        ...
        var countGt2 = 0;
        if(json['todayShopFaultInfoList'] &&json['todayShopFaultInfoList'].length>0 ){
            countGt2 ++;
        }
        ...
        drawTableArea(json['todayShopFaultInfoList'], isShowTitle ? json['todayTotalRows'] : "","今天");
    ...
    function drawTableArea(datas, numberStr,titleDayStr) {
        ...
        if (!G.isEmpty(datas) && !G.isEmpty(datas[0])) {
            ...
            $.each(datas, function (index, value) {
                ...
                var customerId = G.Lang.normalize(value['customerIdStr']);
                var vehicleId = G.Lang.normalize(value['vehicleIdStr']);
                var faultCode = G.Lang.normalize(value['faultCode']);
                var faultCodeCategory = G.Lang.normalize(value['faultCodeCategory']);
                var faultCodeDescription =  G.Lang.normalize(value['faultCodeDescription']);
                var faultInfo = G.Lang.generateString([faultCodeCategory,faultCodeDescription]," ","--");
                var vehicleBrand = G.Lang.normalize(value['vehicleBrand']);
                var vehicleModel = G.Lang.normalize(value['vehicleModel']);
                var vehicleInfo = G.Lang.generateString([vehicleBrand,vehicleModel]," ","--");
                var customerName = G.Lang.normalize(value['customerName']);
                var customerMobile =G.Lang.normalize(value['customerMobile']);
                var lon =G.Lang.normalize(value['lon']);
                var lat =G.Lang.normalize(value['lat']);

                var id = G.Lang.normalize(value['idStr']);
                var appointOrderIdStr = G.Lang.normalize(value['appointOrderIdStr']);
                var customerInfo = G.Lang.generateString([customerName,customerMobile]," ","--");
                var state = "";
                ...
                tr += '    <td class="score_table2_left"><input type="checkbox" data-id="' + id + '" data-vehicle="'+value['vehicleNo']+'"/></td>';
                tr += '    <td>' + value['faultCodeReportTimeStr'] + '</td>';
                tr += '    <td>' + value['faultAlertTypeValue'] + '</td>';
                tr += '    <td><a style="color: #000000;" title="' + faultInfo + '">' + G.normalize(faultCode,"--")+ '<br/>'+(faultInfo.length>16?faultInfo.substr(0,16)+'...':faultInfo)+'</a></td>';
                if(G.Lang.isNotEmpty(vehicleId) && G.Lang.isNotEmpty(customerId)){
                    var vehicleHref = "unitlink.do?method=customer&customerId="+customerId +"&vehicleId="+vehicleId;
                    tr += '    <td title="' + vehicleInfo + '"><a class="blue_color" href="' + vehicleHref + '">' + vehicleNo + '</a><br/>'+(vehicleInfo.length>9?vehicleInfo.substr(0,9)+'...':vehicleInfo)+ '</td>';
                }else{
                    tr += '    <td title="' + vehicleInfo + '">' + vehicleNo+'<br/>'+(vehicleInfo.length>9?vehicleInfo.substr(0,9)+'...':vehicleInfo)+ '</td>';
                }
                ...
                tr += '<td>';
                if (value['isCreateAppointOrder'] == 'YES') {
                    tr += '<a class="blue_color" action="toAppointOrder" data-id="' + appointOrderIdStr + '">查看预约单</a>';
                } else {
                    tr += '<a class="blue_color" action="generateAppointOrder" data-id="' + id + '">生成预约</a>';
                }
...
//                    tr += '<br/><a class="blue_color viewPosition" data-lon="'+lon+'" data-lat="'+lat+'">查看位置</a>';
                }
                tr += '</td>';
                tr += '</tr>';
            });
          ...
          /* $(".viewPosition").each(function(index,element){
