$().ready(function() {
    ...
    mapMark.initDefault({
        ...
        "onSelect":function(event, itemData)  {
            window.location.href="apply.do?method=getApplySuppliersPage&provinceNo="+itemData.provinceNo;
        }
    });

    if ($("#recommendSupplierDiv").length > 0) {
        ...
        function initRecommendSupplier(json) {
            var size = 5;

            if (json == null || json.data == null ||json.data.length<=0 ) {
                $(".J_supplierRecommendShow").hide();
                return;
            }
            ...
            str += '</ul>';
            $("#recommendSupplierDiv").html(str);

            for (var liIndex = 0; liIndex < liSize; liIndex ++) {
                var liHtml = '';
                liHtml += '<div class="cuSearch look_supplier" style=" width: 664px;height: 220px">';
                liHtml += '    <span class="cart_title lookSupplier_title"></span>     ';
                ...
                for (var index = 0; index < size; index ++) {
                    ...
                    tr += '        <a class="blue_color" href="#" onclick="window.open(\'shopMsgDetail.do?method=renderShopMsgDetail&paramShopId=' + idStr + '\', \'_blank\')">'+name+'</a>';
  ...
  $("#searchSupplyBtn").click(function(){
