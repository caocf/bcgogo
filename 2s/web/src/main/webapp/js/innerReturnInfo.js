...
//           tableUtil.setRowBackgroundColor("#table_productNo", null, "#totalRowTR", 'odd');
    ...
    $("#picking_btn").bind("click", function () {
        ...
        if (validateInnerReturn()) {
            $('#innerReturnForm').ajaxSubmit({
                url: "pick.do?method=validatorInnerReturn",
                dataType: "json",
                type: "POST",
                success: function (result) {
                    if (result) {
                        if (result.success) {
                            nsDialog.jConfirm("确认退料？", "内部退料提示", function (returnVal) {
                                if (returnVal) {
                                    $("#innerReturnForm").attr("action", "pick.do?method=saveInnerReturn");
                                    $("#innerReturnForm").submit();
                                }
                            });
                        } else {
    ...
    isShowAddButton();
    setTotal();
    if ($("#innerReturnMessage").val()) {
        nsDialog.jAlert($("#innerReturnMessage").val());
    }

})


function innerReturnAdd() {
    var tr = $(getInnerReturnTrSample()).clone();     //TODO 克隆模版，初始化所有的INPUT
    $(tr).find("input").val("");
...
function getInnerReturnTrSample() {
...
//        '</td>' +
        ...
        '<td>' +
        '<input id="itemDTOs0.unit" name="itemDTOs[0].unit"  value="" class="itemUnit txt checkStringEmpty" style="width:20px;"  type="text"/>' +
        '<input type="hidden" id="itemDTOs0.storageUnit" name="itemDTOs[0].storageUnit" value="" class="itemStorageUnit txt"/>' +
        '<input type="hidden" id="itemDTOs0.sellUnit" name="itemDTOs[0].sellUnit" value="" class="itemSellUnit txt"/>' +
        '<input type="hidden" id="itemDTOs0.rate" name="itemDTOs[0].rate" value="" class="itemRate txt"/>' +
        '</td>' +
...
function setTotal() {
    ...
    $(".itemPrice").each(function (i) {
        ...
        htmlNumberFilter($("#" + idPrefix + "\\.price_span").add("#" + idPrefix + "\\.amount").add("#" + idPrefix + "\\.total_span"), true);
    })
...
//校验单据的信息完整与正确性
function validateInnerReturn() {
    if(!$("#pickingMan").val()){
        nsDialog.jAlert("请选择退料人！");
        return false;
    }
    if(!$("#vestDateStr").val()){
        nsDialog.jAlert("请输入退料时间！");
        return false;
    }
    var msg = "";
    ...
    $(".item").each(function (i) {
        ...
        if (trCount == 1 && checkEmptyRow($(this))) {
            ...
            msg += "<br>第" + ($(this).index()) + "行，商品不存在，请重新选择商品";
        }else if (G.Lang.isEmpty(unit)) {
            msg += "<br>第" + ($(this).index()) + "行，请输入单位.";
        }
    });
