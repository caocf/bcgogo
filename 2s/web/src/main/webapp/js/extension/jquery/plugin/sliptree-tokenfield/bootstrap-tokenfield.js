  ...
  * ============================== */
  ...
  , createToken: function (attrs, triggerChange) {
     ...
     if (typeof triggerChange === 'undefined') {
      ...
      //ndong:add param to token
      if(attrs.params){
          var params=attrs.params;
          beforeCreateEvent.token.params={};
          for(var key in params){
              token.append('<input type="hidden" class="token_param" name="'+key+'" value="'+params[key]+'">');
              beforeCreateEvent.token.params[key]=params[key];
          }
      }
      //是否可以编辑
       if(attrs.editable==false||attrs.editable=="false"){
          token.attr("disabled","disabled");
          token.attr("title","选择的联系人,不可编辑");
       }
      ...
      token
        ...
        .on('dblclick', function (e) {
          if (_self.disabled||$(this).attr("disabled")=="disabled") return false;
          _self.edit( token )
        })
...
//            .on('click',  $.proxy(this.remove, this))
          ...
          //jmc: 新增代码, 不同数据不同处理
          var _dealNotExist = this.options.dealNotExist;
          var _dealExist = this.options.dealExist;
          if(attrs.editable!=false&&attrs.editable!="false"&&isFunction(this.options.checkIfExist)){
              if(this.options.checkIfExist(value, label)){
                  closeButton.addClass("glyphicon glyphicon-pencil").on('click', function(){
                      if (_self.disabled) return false;
                      var tValue="";
//                      if(!$(this).closest(".token").hasClass("token-invalid")){
                          tValue=$(this).closest(".token").attr("data-value");
                         var $token=$(this).closest(".token");

                      _dealExist(tValue, label,$token);
                  });
              }else{
                  closeButton.addClass("glyphicon glyphicon-plus").on('click', function(){
                      _dealNotExist(value, label)
                  });
              }
          }
  ...
  , getTokens: function(active) {
      ...
      this.$wrapper.find( '.token' + activeClass ).each( function() {
          var data=self.getTokenData( $(this) );
          var $token_param=$(this).find(".token_param");
          if($token_param.length>0){
              data.params={};
              $token_param.each(function(){
  ...
  * ======================== */
  ...
  $.fn.tokenfield.defaults = {
    ...
    beautify: true,
      ...
      dealExist: function(value, label){
      }
  }
