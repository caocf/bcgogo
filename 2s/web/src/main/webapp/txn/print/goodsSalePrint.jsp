...
<div class="print_cont">
    <h3>${salesOrderDTO.shopName}</h3>
    <!--第一部分-->
    <div class="i_searchTitle pruch clear">
        <label class="danju">单据号:</label>
        <label class="receipt danju">${salesOrderDTO.id}</label>
        <label>销售单</label>
        <label class="zhidan">制单时间：${salesOrderDTO.editDateStr}</label>
    </div>
    <!--第一部分结束-->

    <!--第二部分-->
    <%--<div class="pruch_use clear">--%>
    <%--<label>客户名:</label><span>${salesOrderDTO.customer}</span><label>联系人:</label><span>${salesOrderDTO.contact}</span><label>联系电话:</label><span class="non_pruch">${salesOrderDTO.mobile}</span><label class="address_lab">地址:</label><span class="address_show">${salesOrderDTO.address}</span>--%>
    <%--<div class="clear"></div>--%>
    <%--</div>--%>
    <!--第二部分结束-->
    ...
    <table class="pruch_tab">
        ...
        <tr>
            <td>客户名:</td>
            <td>${salesOrderDTO.customer}</td>
            <td>联系人:</td>
            <td>${salesOrderDTO.contact}</td>
            <td>联系电话:</td>
            <td>${salesOrderDTO.mobile}</td>
            <td>地址:</td>
            <td>${salesOrderDTO.address}</td>
        </tr>
    </table>
    ...
    <table cellpadding="0" cellspacing="0" class="table2">
        ...
        <tr>
            <td>实收</td>
            <td colspan="2">${salesOrderDTO.settledAmount}¥</td>
            <td>实收(大写)：</td>
            <td colspan="6" class="font_set">${salesOrderDTO.settledAmountStr}</td>
        </tr>
    </table>
    <div class="clear" style="height:10px;"></div>
    <div>备注：${salesOrderDTO.memo}</div>
    <!--施工单结束-->
    <div class="qianzi clear">
        <div>销售人签字：<label></label></div>
    	<span class="print_num">店长签字：
    	<label></label></span>
    ...
    <div class="address clear">
        <div class=" time time_p"><span>地址：</span><label>${salesOrderDTO.shopAddress}</label></div>
        <div class=" phone_time">电话：<label>${salesOrderDTO.shopLandLine}
            <c:if test="${storeManagerMobile != null && storeManagerMobile !=''}">
                <c:if test="${salesOrderDTO.shopLandLine != null && salesOrderDTO.shopLandLine != ''}">
                    ,
                </c:if>
                ${storeManagerMobile}
            </c:if>
