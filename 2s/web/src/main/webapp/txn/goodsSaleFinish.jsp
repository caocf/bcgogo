...
$().ready(function(){
    ...
    $("#dispatchConfirmBtn").bind("click", function () {
        ...
        if($("#company").val()!=""){
            ...
            setTimeout(function () {
                window.location = "sale.do?method=toOnlineSalesOrder&salesOrderId=${salesOrderDTO.id}";
            }, 5000);
...
<ul class="yinye_title clear">
    <li class="danju" style="width:250px;">
        单据号： <strong>${salesOrderDTO.receiptNo} <a href="javascript:showOrderOperationLog('${salesOrderDTO.id}','sale')"  class="blue_col">操作记录</a></strong>
    </li>
    <c:if test="${!empty salesOrderDTO.purchaseOrderId}">
...
<table class="clear tb_tui">
    ...
    <tr>
        ...
        <td><span id="customerName" class="ellipsis" title="${salesOrderDTO.customer}">${salesOrderDTO.customer}</span>
        <c:if test="${not empty customerDTO.customerShopId}">
            ...
            <a class="J_QQ" data_qq="${customerDTO.qqArray}"></a></td>
        </c:if>
    ...
    <c:if test="${!wholesalerVersion}">
        <tr>
            ...
            <td>${salesOrderDTO.licenceNo}</td>
            <td class="td_title">车主</td>
            <td>${salesOrderDTO.vehicleContact}</td>
            <td class="td_title">车主手机</td>
            <td>${salesOrderDTO.vehicleMobile}</td>
        </tr>
    </c:if>
    ...
    <tr>
        ...
        <bcgogo:hasPermission resourceType="logic" permissions="WEB.VERSION.STOREHOUSE">
            <c:if test="${!(salesOrderDTO.status eq 'PENDING')}">
                <td style="width:12%;">仓库：${salesOrderDTO.storehouseName} </td>
            </c:if>
        </bcgogo:hasPermission>
        <td style="width:10%;">
            <div style="float:left;width:160px;" id="goodsSalerDiv">
                销售人：
                <c:choose>
                    ...
                    <c:otherwise>
                        ${salesOrderDTO.goodsSaler}
                    </c:otherwise>
                </c:choose>
            </div>
        </td>
        <td style="width:17%;">销售日期：${salesOrderDTO.vestDateStr}</td>
        <td style="width:17%;">
            <c:if test="${! empty purchaseOrderDTO.deliveryDateStr}">
                买家期望交货:${purchaseOrderDTO.deliveryDateStr}
            </c:if>
        </td>
        <td style="width:17%;">
            <c:if test="${! empty salesOrderDTO.preDispatchDateStr}">
                预计交货日期:${salesOrderDTO.preDispatchDateStr}
            </c:if>
        </td>
        <td>
            <c:if test="${!empty salesOrderDTO.purchaseOrderId}">
...
<table class="clear tb_tui">
    ...
    <col width="95"/>
    <col width="95"/>
    <c:if test="${salesOrderDTO.purchaseOrderId!=null}">
        <col width="80">
    </c:if>
    <col width="80"/>
    <col width="95"/>
    <tr class="tab_title">
        <td style="width:300px;">商品编号</td>
        <td>品名</td>
        <td>品牌/产地</td>
        <td>规格</td>
        <td>型号</td>
    ...
    <c:forEach items="${salesOrderDTO.itemDTOs}" var="itemDTO" varStatus="status">
        <c:if test="${itemDTO!=null}">
            <tr class="item">
                <td>
                    <span style="display:none" class="j_supplierProductId">${itemDTO.productId}</span>
                ...
                <c:choose>
                    <c:when test="${salesOrderDTO.status eq 'STOCKING' && itemDTO.shortage>0}">
                        <td>
                            <fmt:formatNumber value="${itemDTO.amount}" pattern="#.##"/>
    ...
    <tr>
        <td colspan="11">合计：</td>
        <td><span><fmt:formatNumber value="${salesOrderDTO.amountTotal}" pattern="#.##"/></span></td>
        <td colspan="2">
            <span><fmt:formatNumber value="${salesOrderDTO.itemTotal}" pattern="#.##"/></span>
        </td>
    </tr>
    <tr>
        <c:if test="${!empty salesOrderDTO.purchaseOrderId}">
            <td class="td_title">采购备注：</td>
            <td colspan="13"><span class="sale-memo">${salesOrderDTO.purchaseMemo}</span></td>
        </c:if>
        <c:if test="${empty salesOrderDTO.purchaseOrderId}">
            <td class="td_title"> 备注：</td>
            <td colspan="13"><span class="sale-memo">${salesOrderDTO.memo}</span></td>
        </c:if>
    </tr>
</table>
    <c:if test="${!empty salesOrderDTO.purchaseOrderId}">
...
<c:if test="${salesOrderDTO.otherIncomeItemDTOList != null}">
...
<table class="clear tb_tui">
    ...
    <tr>
        <td colspan="2">合计：</td>
        <td >${salesOrderDTO.otherIncomeTotal}</td>
    </tr>
...
<c:if test="${salesOrderDTO.status eq 'SALE_DONE' || salesOrderDTO.status eq 'SALE_DEBT_DONE' || (salesOrderDTO.status eq 'SALE_REPEAL' && salesOrderDTO.receivableId!=null)}">
    <div class="height"></div>
    <strong class="jie_info clear">结算信息</strong>
        <div class="jie_detail clear">
          <div class="almost" style="float:left">
          <c:if test="${salesOrderDTO.memberDiscountRatio != null}" var="isMember">
           <div>应收总额：&nbsp;<span class="borders">${salesOrderDTO.total}元</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${salesOrderDTO.memberDiscountRatio}折后价格：<span>${salesOrderDTO.afterMemberDiscountTotal}</span>元</div>
            </c:if>
          <c:if test="${!isMember}">
           <div>应收总额：&nbsp;<span>${salesOrderDTO.total}元</span></div>
                    </c:if>
            <div style="">实收总计：&nbsp;<span>${salesOrderDTO.settledAmount}元</span></div>
            <div style="">优惠总计：&nbsp;<span>
              <c:if test="${salesOrderDTO.memberDiscountRatio != null}" var="isMember">
            ...
            <div style="">挂账金额：&nbsp;<span class="red_color" id="orderDebt">${salesOrderDTO.debt}元</span></div>
           </div>
            <c:if test="${not empty salesOrderDTO.promotionsInfoDTO}">
            <div class="orderPromotionsDetail" style="float:left;margin-left:25px;">
                <div class="infoName clear" style="font-size: 13">优惠明细：</div>
                <c:if test="${not empty salesOrderDTO.promotionsInfoDTO.BARGAIN}">
                <div class="bargainDiv clear">
                    <span>特价商品：</span>
                ...
                <div class="mjsDiv clear">
                    <span>满就送：</span>
                    赠送<span id="mjsSpan">${salesOrderDTO.promotionsInfoDTO.MJS}</span>
                </div>
                </c:if>
                <c:if test="${not empty salesOrderDTO.promotionsInfoDTO.FREE_SHIPPING}">
                <div class="freeShippingDiv clear">
...
<div class="shopping_btn" style="float:right; clear:right;">
    <bcgogo:hasPermission permissions="WEB.TXN.SALE_MANAGE.SALE" resourceType="menu">
        <c:if test="${salesOrderDTO.status eq 'PENDING'}">
            <div class="btn_div_Img" id="accept_div" onclick="acceptSale()">
    ...
    <bcgogo:hasPermission permissions="WEB.TXN.SALE_MANAGE.RETURN">
        <c:if test="${(salesOrderDTO.status eq 'SALE_DONE' || salesOrderDTO.status eq 'SALE_DEBT_DONE') && empty salesOrderDTO.purchaseOrderId}">
            <div class="salesReturn_div" id="salesReturnDiv" style="float:left;">
...
<div class="i_searchBrand" id="acceptDialog" title="确认接受提示" style="display:none; width:500px;">
    ...
    <form id="salesAcceptForm" method="post" action="sale.do?method=acceptSaleOrder">
        <table border="0" width="500">
            ...
            <tr>
                ...
                <td><span id="pruchaseVestDate">${purchaseOrderDTO.deliveryDateStr}</span></td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    <input id="dispatchDate" type="hidden" name="preDispatchDate"/>
    ...
    <form id="dispatchForm" method="post" action="sale.do?method=dispatchSaleOrder">
        <table border="0" width="480">
            ...
            <tr>
                <td colspan="2">
                    <div class="btnClick" style="height:50px; line-height:50px">
                        <input type="hidden" id="dispatch_id" name="id" value="${salesOrderDTO.id}"/>
