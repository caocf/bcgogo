...
<head>
    ...
    <script type="text/javascript" src="js/innerPickingInfo<%=ConfigController.getBuildVersion()%>.js"></script>
...
<div class="i_main clear">
    ...
    <div id="i_mainRight" class="i_mainRight shoppingCart">
        ...
        <div class="cartBody ">
            <input type="hidden" id="orderType" value="innerPicking">
            <form:form action="pick.do?method=saveInnerPicking" id="innerPickingForm" commandName="innerPickingDTO" method="post" class="J_leave_page_prompt">
                <%--<c:if test="${innerPickingDTO.receiptNo}">--%>
                <%--<div class="divTit">--%>
                <%--单据号：<span>${innerPickingDTO.receiptNo}</span>--%>
                ...
                <div id="pickingManDiv" class="divTit" style="width:174px;">领料人：<form:input path="pickingMan" readonly="true" cssClass="txt"/>
                ...
                <div class="divTit">
                    领料日期：<form:input path="vestDateStr" readonly="true" cssClass="txt" style="width:130px;"/>
                </div>
                <div class="divTit">
                    操作人：<span>${innerPickingDTO.operationMan}</span>
                </div>
                ...
                <table cellpadding="0" cellspacing="0" class="table2" id="table_productNo">
                    ...
                    <c:forEach items="${innerPickingDTO.itemDTOs}" var="itemDTO" varStatus="status">
                        <tr class="item table-row-original">
                                ...
                                <bcgogo:permission>
                                    <bcgogo:if resourceType="logic" permissions="WEB.VERSION.PRODUCT.THROUGH_SELECT_SUPPLIER">
                                        <form:input path="itemDTOs[${status.index}].amount" value='0'
                                                    class="txt checkNumberEmpty itemAmount" title='${itemDTO.amount}' maxlength="20"
                                                    cssStyle="width: 30px;"/>
                                    </bcgogo:if>
                                    <bcgogo:else>
                                        <form:input path="itemDTOs[${status.index}].amount" value='${itemDTO.amount}'
                                                    class="txt checkNumberEmpty itemAmount" title='${itemDTO.amount}' maxlength="20"
                                                    cssStyle="width: 30px;" data-filter-zero="true"/>
                                    </bcgogo:else>
                                </bcgogo:permission>
                                /
                            <span id="itemDTOs${status.index}.inventoryAmountSpan"
                                  name="itemDTOs[${status.index}].inventoryAmountSpan"
                                  title="${itemDTO.inventoryAmount}">${itemDTO.inventoryAmount}</span>
                                <form:hidden path="itemDTOs[${status.index}].inventoryAmount"
                                             value='${itemDTO.inventoryAmount}'/>
                            </td>
                            <td>
                            ...
                            <td>
                                <a id="itemDTOs${status.index}.btnMinus" class="btnMinus"
                                   name="itemDTOs[${status.index}].btnMinus">删除</a>
                                <a id="itemDTOs${status.index}.btnPlus" class="btnPlus"
                                   name="itemDTOs[${status.index}].btnPlus">增加</a>
                            </td>
                        </tr>
                    </c:forEach>
                    <tr style="font-weight:bold;border-top:1px solid #AAAAAA" id="totalRowTR">
                        ...
                        <td>
                            <span id="totalItemAmount_span" data-filter-zero="true"></span>/<span id="totalInventoryAmount_span" data-filter-zero="true"></span>
                        </td>
                        <td>
                            <span id="total_span" data-filter-zero="true">${innerPickingDTO.total}</span>
        ...
        <div class="shopping_btn">
            ...
            <div class="btn_div_Img">
                <input id="show_innerPicking_list_btn" class="return_list j_btn_i_operate" type="button" onfocus="this.blur();">
