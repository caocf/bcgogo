...
<head>
    ...
    <script type="text/javascript" src="js/innerReturnList<%=ConfigController.getBuildVersion()%>.js"></script>
...
<div class="i_main clear">
    ...
    <div class="titBody ">
        <form:form action="pick.do?method=showInnerReturnListPage" id="thisform" name="thisform" method="post" commandName="searchCondition">
            ...
            <div class="allocation">
                ...
                <div class="allocation_center">
                    <div class="divTit">退料单号：<form:input path="receiptNo" class="txt"/></div>
                    <c:if test="${searchCondition.isHaveStoreHouse}">
                    ...
                    <div class="divTit">退料日期：<form:input path="startTimeStr" readonly="true" cssClass="txt"/>&nbsp;至&nbsp;
        ...
        <div class="wordTitle">
            共有<span>${totalAmount}</span>条内部退料记录
            <bcgogo:hasPermission permissions="WEB.INNER_RETURN.ADD">
                <input type="button" value="新增内部退料" class="addNew" id="addNewInnerReturn"/>
        ...
        <table cellpadding="0" cellspacing="0" class="tabSlip tabPick">
            ...
            <tr class="divSlip">
                ...
                <th>单据号</th>
                <bcgogo:hasPermission permissions="WEB.TXN.INVENTORY_MANAGE.ALLOCATE_RECORD" resourceType="menu">
                    <th>退料仓库<input id="sortStorehouseName" class="ascending jsp_sort" type="button" onfocus="this.blur();" sortField="storehouseName"></th>
                </bcgogo:hasPermission>
                <th>退料人<input id="sortPickingMan" class="ascending jsp_sort" type="button" onfocus="this.blur();" sortField="pickingMan"></th>
                <th>退料数量合计</th>
                <th>退料成本合计<input id="sortTotal" class="ascending jsp_sort" type="button" onfocus="this.blur();" sortField="total"></th>
                <th>操作人<input id="sortOperationMan" class="ascending jsp_sort" type="button" onfocus="this.blur();" sortField="operationMan"></th>
                <th>退料日期<input id="sortVestDate" class="descending jsp_sort" type="button" onfocus="this.blur();" sortField="vestDate"></th>
            </tr>
            <c:forEach items="${innerReturnDTOs}" var="innerReturnDTO" varStatus="status">
                <tr style="cursor: pointer;text-align: center;">
                    <td style="padding-left:10px;">${status.index+1}</td>
                    <td><a class="blue_col showInnerReturn" url="pick.do?method=showInnerReturn&innerReturnId=${innerReturnDTO.id}">${innerReturnDTO.receiptNo}</a></td>
                    <bcgogo:hasPermission permissions="WEB.TXN.INVENTORY_MANAGE.ALLOCATE_RECORD" resourceType="menu">
                    <td>${innerReturnDTO.storehouseName}</td>
                    </bcgogo:hasPermission>
                    <td>${innerReturnDTO.pickingMan}</td>
                    <td>${innerReturnDTO.totalAmount}</td>
                    <td>${innerReturnDTO.total}</td>
                    <td>${innerReturnDTO.operationMan}</td>
                    <td>${innerReturnDTO.vestDateStr}</td>
                </tr>
            </c:forEach>
        </table>
        <c:if test="${empty innerReturnDTOs}">
            <br>
        ...
        <div>
            <jsp:include page="/common/paging.jsp">
                <jsp:param name="url" value="pick.do?method=showInnerReturnListPage"/>
