 ...
 */
@Component
public class CustomerApplyGuideValidator extends AbstractValidator{
  ...
  protected UserGuideDTO validate(long shopVersionId, long shopId, long userId, UserGuideDTO userGuideDTO, boolean needNext, String... excludeFlowName) throws Exception {
    //前三步需要检查
    if ("CUSTOMER_APPLY_GUIDE_BEGIN,CUSTOMER_APPLY_GUIDE_ENTER_MESSAGE,CUSTOMER_APPLY_GUIDE_SHOW_APPLY".contains(userGuideDTO.getCurrentStep().getName()) && !hasSupplierApplies(shopId)) {
