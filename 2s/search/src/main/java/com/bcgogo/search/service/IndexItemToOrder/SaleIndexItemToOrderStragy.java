 ...
 */
public class SaleIndexItemToOrderStragy implements IndexItemToOrderStragy {
  @Override
  public void indexItemToOrder(ItemIndexDTO itemIndexDTO, OrderDTO orderDTO) {
    ...
    if(itemIndexDTO.getOrderTimeCreated()!=null){
    ...
    /*if(OrderStatus.SALE_REPEAL == itemIndexDTO.getOrderStatus()){
        orderDTO.setStatus(OrderStatus.SALE_REPEAL);
    } else{
        orderDTO.setStatus(OrderStatus.SALE_DONE);
    }*/
    orderDTO.setTotalMoney(itemIndexDTO.getOrderTotalAmount());
    orderDTO.setVehicle(itemIndexDTO.getVehicle());
    orderDTO.setUrl("sale.do?method=toSalesOrder&menu-uid=WEB.TXN.SALE_MANAGE.SALE&salesOrderId=" + itemIndexDTO.getOrderId().toString());
  }
}
