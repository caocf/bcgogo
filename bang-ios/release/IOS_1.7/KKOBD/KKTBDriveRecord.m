...
-(BOOL) insertRecord:(BGDriveRecordDetail *)aDriveRecordDetail
{
    ...
    NSString *sql = [NSString stringWithFormat:@"insert into %@ values (%lld,%@,%@,%lld,%@,%lld,%@,%@,%@,%lld,%@,%@,%@,%d,%2f,%2f,%2f,%@,%2f,%@,%@,%@,%d) ",tb_Name,
...
{
    NSString *sql = [NSString stringWithFormat:@"update %@ set id = %lld , lastUpdateTime = %lld , status = %@ , state = %d where appDriveLogId = %@",tb_Name,aDetail.id,aDetail.lastUpdateTime,nilOrString(aDetail.status),aDetail.state,nilOrString(aDetail.appDriveLogId)];
...
-(NSArray *) queryDriveRecordWithSQL:(NSString *)sql
{
    ...
    @try {
		...
		while ([rs next])
		{
            ...
            detail.distance = [rs intForColumn:@"distance"];
            detail.oilWear = [rs intForColumn:@"oilWear"];
            detail.oilPrice = [rs intForColumn:@"oilPrice"];
            detail.oilKind = [rs stringForColumn:@"oilKind"];
            detail.totalOilMoney = [rs intForColumn:@"totalOilMoney"];
...
{
    NSString *sql = [NSString stringWithFormat:@"update %@ set endPlace = '%@' where appDriveLogId = '%@'",tb_Name,aEndPlace,aAppDriveLogId];
