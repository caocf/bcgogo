...
const NSString *OBDManufacturerString_QiQu = @"QiQu";

NSString *KKOBDDeviceInfoServiceUUID = @"180a";
NSString *KKOBDSystemIdCharacteristicUUID = @"2a23";
NSString *KKOBDManufacturerCharacteristicUUID = @"2a29";

const NSString *KKOBDWriteServiceUUID = @"ffe5";
const NSString *KKOBDWriteCharacteristicUUID = @"ffe9";
const NSString *KKOBDReadServiceUUID = @"ffe0";
const NSString *KKOBDReadCharacteristicUUID = @"ffe4";

const NSString *QQOBDWriteServiceUUID = @"18f0";//@"ffe5";
const NSString *QQOBDWriteCharacteristicUUID = @"2af1";//@"ffe9";
const NSString *QQOBDReadServiceUUID = @"18f0";//@"ffe0";
const NSString *QQOBDReadCharacteristicUUID = @"2af0";//@"ffe4";
...
- (void)writeData:(NSData *)aData
{
    ...
    NSInteger characteristicWriteType = CBCharacteristicWriteWithResponse;
    switch (self.obdManufacturer) {
        case OBDManufacturer_QiQu:
            characteristicWriteType = CBCharacteristicWriteWithoutResponse;
            break;
    ...
    if(_writeCharacteristic)
        [_activePeripheral writeValue:aData forCharacteristic:_writeCharacteristic type:characteristicWriteType];
    else
    {
        [self disConnectActivePeripheral];
        [self scanPeripherals:8];
    }
}
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central
{
    ...
    if([self.bleDelegate respondsToSelector:@selector(centralManagerDidUpdateState:)])
        [self.bleDelegate centralManagerDidUpdateState:central.state];
}
...
//    [_scanPeripherals insertObject:peripheral atIndex:pos];
    [_scanPeripherals addObject:peripheral];
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral
{
    ...
    if(self.obdManufacturer == OBDManufacturer_Unknow)
    {
        [_activePeripheral discoverServices:@[ [CBUUID UUIDWithString:KKOBDDeviceInfoServiceUUID]]];
    }
    else
    {
...
- (void)peripheral:(CBPeripheral *)aPeripheral didDiscoverServices:(NSError *)error
{
    ...
    for (CBService *service in aPeripheral.services) {
        ...
        if ([service.UUID isEqual:[CBUUID UUIDWithString:KKOBDDeviceInfoServiceUUID]]) {  // device information
            if(self.obdManufacturer == OBDManufacturer_Unknow)
            {
                [_activePeripheral discoverCharacteristics:@[[CBUUID UUIDWithString:KKOBDManufacturerCharacteristicUUID]] forService:service];
            }
            else
            {
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error
{
    ...
    if (self.obdManufacturer != OBDManufacturer_Unknow && ([service.UUID isEqual:[CBUUID UUIDWithString:self.writeServiceUUID]] ||
        [service.UUID isEqual:[CBUUID UUIDWithString:self.readServiceUUID]])) {
    ...
    if ([service.UUID isEqual:[CBUUID UUIDWithString:KKOBDDeviceInfoServiceUUID]]) {
        for (CBCharacteristic *characteristic in service.characteristics) {
            if(self.obdManufacturer == OBDManufacturer_Unknow && [characteristic.UUID isEqual:[CBUUID UUIDWithString:KKOBDManufacturerCharacteristicUUID]])
            {
                [peripheral readValueForCharacteristic:characteristic];
                break;
            }
            if(self.obdManufacturer != OBDManufacturer_Unknow && [characteristic.UUID isEqual:[CBUUID UUIDWithString:KKOBDSystemIdCharacteristicUUID]]) {
                // system id
                [peripheral readValueForCharacteristic:characteristic];
                break;
            }
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    if([characteristic.UUID isEqual:[CBUUID UUIDWithString:KKOBDManufacturerCharacteristicUUID]])
    {
        NSString *manStr = [[NSString alloc] initWithData:characteristic.value encoding:NSUTF8StringEncoding];
        if([manStr isEqualToString:(NSString *)OBDManufacturerString_QiQu])
        {
            self.obdManufacturer = OBDManufacturer_QiQu;
            
            [self setWriteServiceUUID:(NSString *)QQOBDWriteServiceUUID];
            [self setWriteCharacteristicUUID:(NSString *)QQOBDWriteCharacteristicUUID];
            [self setReadServiceUUID:(NSString *)QQOBDReadServiceUUID];
            [self setReadCharacteristicUUID:(NSString *)QQOBDReadCharacteristicUUID];
        }
        else
        {
            self.obdManufacturer = OBDManufacturer_Old;
            
            [self setWriteServiceUUID:(NSString *)KKOBDWriteServiceUUID];
            [self setWriteCharacteristicUUID:(NSString *)KKOBDWriteCharacteristicUUID];
            [self setReadServiceUUID:(NSString *)KKOBDReadServiceUUID];
            [self setReadCharacteristicUUID:(NSString *)KKOBDReadCharacteristicUUID];
        }
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:KKOBDSystemIdCharacteristicUUID]]) {
        ...
        switch (self.obdManufacturer) {
            case OBDManufacturer_Old:
                systemId = [NSString stringWithFormat:@"%02X:%02X:%02X:%02X:%02X:%02X", sysData[0], sysData[1], sysData[2], sysData[5], sysData[6], sysData[7]];
                break;
            case OBDManufacturer_QiQu:
                systemId = [NSString stringWithFormat:@"%02X:%02X:%02X:%02X:%02X:%02X", sysData[0], sysData[1], sysData[2], sysData[3], sysData[4], sysData[5]];
                break;
            default:
                break;
        }
