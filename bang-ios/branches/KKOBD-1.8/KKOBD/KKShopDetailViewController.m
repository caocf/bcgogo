...
- (void) initVariables
{
    self.commentArray = [[[NSMutableArray alloc] init] autorelease];
    _page = 1;
    _pageSize = 10;
    _enableRefresh = YES;
}
...
- (void)addShopDetailView
{
    ...
    if ([self.detailInfo.mobile length] > 0 || [self.detailInfo.landLine length] > 0)
    {
        ...
        UIButton *phoneBtn = [[UIButton alloc] initWithFrame:CGRectMake(16, startPoint.y, 140, image.size.height)];
        ...
        UILabel *phoneNumLabel = [[UILabel alloc] initWithFrame:CGRectMake(44, 27, 110, 12)];
...
//        startPoint.y += 6;
    ...
    UIButton *onLineBtn = [[UIButton alloc] initWithFrame:CGRectMake(168, startPoint.y, 140, image.size.height)];
    ...
    UILabel *rateLb = [[UILabel alloc] initWithFrame:CGRectMake(startPoint.x, startPoint.y, 200, 15)];
    rateLb.backgroundColor = [UIColor clearColor];
    rateLb.textColor = [UIColor blackColor];
    rateLb.font = [UIFont systemFontOfSize:15.f];
    rateLb.textAlignment = UITextAlignmentLeft;
    rateLb.text = [NSString stringWithFormat:@"店面评价(共%d条)",self.commentCount];
    [_mainScrollView addSubview:rateLb];
    [rateLb release];
    ...
    _rateTableView = [[UITableView alloc] initWithFrame:CGRectMake(startPoint.x, startPoint.y, 288, currentScreenHeight - 44 - 49 - [self getOrignY])];
    _rateTableView.delegate = self;
    _rateTableView.dataSource = self;
    _rateTableView.scrollEnabled = NO;
    _rateTableView.bounces = YES;
    [_mainScrollView addSubview:_rateTableView];
    [_rateTableView release];
    ...
    {
//        CGRect rect = _rateTableView.frame;
////        if(self.commentCount * 46 < rect.size.height)
////        {
//////            rect.size.height = self.commentCount * 46;
//////            _rateTableView.frame = rect;
////        }
//        startPoint.y += rect.size.height;
    }
    
    if(self.commentCount > _pageSize)
    {
        _enableRefresh = YES;
        _page = 2;
    }
    ...
    {
        if(self.commentCount <= _pageSize && _page == 1)
        {
            CGFloat height = 0;
            for(KKModelComment *comment in self.commentArray)
            {
                height += [KKShopRateTableViewCell calculateVehicleConditionTableViewCellHeightWithContent:comment];
                
            }
            
            if(height > currentScreenHeight - 44 - 49 - [self getOrignY])
            {
                height = currentScreenHeight - 44 - 49 - [self getOrignY];
            }
...
-(void) getShopComment
{
    [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    _isLoading = YES;
    [[KKProtocolEngine sharedPtlEngine] shopCommentListWithId:self.shopId pageNo:_page pageSize:_pageSize delegate:self];
}
...
-(void) scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    {
       // NSLog(@"%@,%@",NSStringFromCGPoint(_mainScrollView.contentOffset),NSStringFromCGPoint(_rateTableView.frame.origin));
        if(fabs(_mainScrollView.contentOffset.y - _rateTableView.frame.origin.y) < 5)
        {
            _rateTableView.scrollEnabled = YES;
        }
        else
        {
            _rateTableView.scrollEnabled = NO;
    ...
    else if(scrollView == _rateTableView)
    {
...
{
    CGFloat height = [KKShopRateTableViewCell calculateVehicleConditionTableViewCellHeightWithContent:[self.commentArray objectAtIndex:indexPath.row]];
...
-(KKShopRateTableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    static NSString *identifier = @"rateTableCell";
    KKShopRateTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if(cell == nil)
    {
        cell = [[[KKShopRateTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier] autorelease];
    }
    
    [cell setCommentAndRefreshUI:[self.commentArray objectAtIndex:indexPath.row]];
...
- (NSNumber *)shopDetailResponse:(NSNumber *)aReqId withObject:(id)aRspObj
{
    ...
    self.commentCount = detailShopInfo.commentCount;
    [self.commentArray addObjectsFromArray:detailShopInfo.appShopCommentDTOs__KKModelComment];
...
-(NSNumber *) shopCommentListResponse:(NSNumber *)aReqId withObject:(id)aRspObj
{
    [MBProgressHUD hideHUDForView:self.view animated:YES];
    NSObject *rsp = (NSObject*)aRspObj;
	if ([rsp isKindOfClass:[KKError class]]) {
    ...
    KKModelShopCommentRsp *commentRsp = (KKModelShopCommentRsp *)rsp;
    
    if(commentRsp.header.code == eRsp_succeed)
    {
        if([commentRsp.results__KKModelComment count] > 0)
            [self.commentArray addObjectsFromArray:commentRsp.results__KKModelComment];
        if(commentRsp.pager.isLastPage)
        {
            _enableRefresh = NO;
        }
        
        _page = commentRsp.pager.nextPage;
