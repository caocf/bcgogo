...
//        self.backgroundColor = [UIColor lightGrayColor];
        ...
        _editBtn.frame = CGRectMake(248, 44, 62, 32);
...
-(void) refreshUIWithDriveRecordDetail:(BGDriveRecordDetail *)aDetail selected:(BOOL)selected indexPath:(NSIndexPath *)aIndexPath
{
    ...
    _distanceLabel.text = [NSString stringWithFormat:@"%.1f公里",aDetail.distance];
    _oilPriceLabel.text = [NSString stringWithFormat:@"%.1f",aDetail.totalOilMoney];
...
{
    if(self.delegate && [self.delegate respondsToSelector:@selector(driveRecordEditBtnClicked:)])
    {
        [self.delegate driveRecordEditBtnClicked:self.indexPath];
    }
}
