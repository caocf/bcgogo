...
+ (KKProtocolAbstractParser*) createParser:(NSInteger)aProtocolApiID
{
	...
	switch (aProtocolApiID) {
        ...
        case ePtlApi_vehicle_fault:
            ...
            break;
        case ePtlApi_vehicle_faultCodeList:
            parser = [[[KKVehicleFaultCodeListRspParser alloc] init] autorelease];
            break;
        case ePtlApi_vehicle_faultCodeOperate:
            parser = [[[KKModelProtocolRspParser alloc] init] autorelease];
            break;
        ...
        case ePtlApi_shop_detail:
            ...
            break;
        case ePtlApi_shop_commentList:
            parser = [[[KKShopCommentListRspParser alloc] init] autorelease];
            break;
        ...
        case ePtlApi_oil_stationList:
            parser = [[[KKOilStationListRspParser alloc] init] autorelease];
            break;
        case ePtlApi_Register_SuggestVehicle:
            parser = [[[KKSuggestionVehicleRspParser alloc] init] autorelease];
            break;
        ...
        case ePtlApi_violate_juheAreaList:
            parser = [[[KKViolateAreaListRspParser alloc] init] autorelease];
            break;
            
        case ePtlApi_violate_query:
            parser = [[[KKViolateQueryResultRspParser alloc] init] autorelease];
            break;
        case ePtlApi_driveRecord_upload:
            parser = [[[BGDriveRecordUploadParser alloc] init] autorelease];
            break;
        case ePtlApi_appUserConfig_update:
            parser = [[[KKModelProtocolRspParser alloc] init] autorelease];
            break;
        default:
            break;
...
-(id) parseObjectWithDict:(NSDictionary *)dict classname:(NSString *)classname
{
    ...
    for (NSInteger i=0; i<outCount; i++) {
        ...
        if([keyName isEqualToString:@"startTime"])
        {
            NSLog(@"%@",reservedKeyName);
        }
...
@implementation KKModelProtocolRspParser
- (id)parse:(id)jsonObject
{
...
- (id)parse:(id)jsonObject
{
    ...
    KKModelSaveVehicleInfoRsp *rsp = [self parseObjectWithDict:dict classname:@"KKModelSaveVehicleInfoRsp"];
    rsp.header = rspHeader;
...
@implementation KKVehicleFaultCodeListRspParser

-(id)parse:(id)jsonObject
{
    KK_PARSE_PREPARE(jsonObject, @"vehicle fault parse error")
    
    KKFaultCodeListRsp *rsp = [self parseObjectWithDict:dict classname:@"KKFaultCodeListRsp"];
    rsp.header = rspHeader;
    return rsp;
}
...
@implementation KKShopCommentListRspParser
- (id)parse:(id)jsonObject
{
    KK_PARSE_PREPARE(jsonObject, @"shop comment parse error")
    
    KKModelShopCommentRsp *rsp = [self parseObjectWithDict:dict classname:@"KKModelShopCommentRsp"];
    rsp.header = rspHeader;
...
@implementation KKSuggestionVehicleRspParser

-(id) parse:(id)jsonObject
{
    KK_PARSE_PREPARE(jsonObject, @"update default vehicle parse error")
    KKModelSuggestionVehicleRsp *rsp = [self parseObjectWithDict:dict classname:@"KKModelSuggestionVehicleRsp"];
    rsp.header = rspHeader;
...
@implementation KKOilStationListRspParser
- (id)parse:(id)jsonObject
{
    KKModelOilStationListRsp *rsp = [self parseObjectWithDict:(NSMutableDictionary *)jsonObject classname:@"KKModelOilStationListRsp"];
...
@implementation KKViolateAreaListRspParser

-(id) parse:(id)jsonObject
{
    KK_PARSE_PREPARE(jsonObject, @"update default vehicle parse error")
    
    KKViolateAreaListRsp *rsp = [self parseObjectWithDict:dict classname:@"KKViolateAreaListRsp"];
    return rsp;
}
@end

@implementation KKViolateQueryResultRspParser

-(id) parse:(id)jsonObject
{
    KK_PARSE_PREPARE(jsonObject, @"update default vehicle parse error")
    
    KKViolateResultRsp *rsp = [self parseObjectWithDict:dict classname:@"KKViolateResultRsp"];
    return rsp;
}

@end

@implementation KKViolateVehicleTypeParser

-(id) parse:(id)jsonObject
{
    NSString *jsonStr = @"{\"result\":[{\"car\":\"大型车\",\"id\":\"01\"},{\"car\":\"小型车\",\"id\":\"02\"},{\"car\":\"使馆汽车\",\"id\":\"03\"},{\"car\":\"领馆汽车\",\"id\":\"04\"},{\"car\":\"境外汽车\",\"id\":\"05\"},{\"car\":\"外籍汽车\",\"id\":\"06\"},{\"car\":\"教练汽车\",\"id\":\"16\"}]}";
    id jsonObj = [jsonStr mutableObjectFromJSONString];
    
    KK_PARSE_PREPARE(jsonObj, @"update default vehicle parse error")
    
    KKViolateVehicleTypeRsp *rsp = [self parseObjectWithDict:dict classname:@"KKViolateVehicleTypeRsp"];
    return rsp;
}

@end

@implementation BGDriveRecordUploadParser

-(id) parse:(id)jsonObject
{
    KK_PARSE_PREPARE(jsonObject, @"user feedback parse error")
    
    BGDriveRecordUploadRsp *rsp = [self parseObjectWithDict:dict classname:@"BGDriveRecordUploadRsp"];
    rsp.header = rspHeader;
    return rsp;
}
