...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(disConnectedPeripheral) name:@"disConnectedPeripheralNotication" object:nil];
...
- (void) initComponents
{
    ...
    [self showVehicleConditionView];
}

-(void) showVehicleConditionView
{
    if ([_dataArray count] == 0)
    {
        if (KKAppDelegateSingleton.connectStatus == e_CarNotOnLine)
        {
            if(_linkWellPromptView != nil)
           {
               [_linkWellPromptView removeFromSuperview];
               _linkWellPromptView =  nil;
           }
...
- (void)clearButtonClicked
{
    if ([_dataArray count] > 0)
    {
        ...
        [alertView addButtonWithTitle:@"确定" imageName:@"alert-blue2-button.png" block:^{
            ...
            {
                KKAppDelegateSingleton.connectStatus = e_CarWell;
            }
            else
            {
