...
- (void)addScrollView
{
    ...
    if(![KKAuthorization sharedInstance].accessAuthorization.scanShop && ![KKAuthorization sharedInstance].accessAuthorization.searchCar)
    {
        [self hideButtonClicked:hideBtn];
    }
}
...
- (void)sureButtonClicked
{
    ...
    if(![KKHelper KKHElpRegexMatchForVehicleNo:vehicleNum])
    {
    ...
    {
//        if(![self.vehicleDetailInfo.vehicleNo isEqualToString:vehicleNum])
//        {
//            self.vehicleDetailInfo.isDefault = @"NO";
//        }
        KKModelVehicleDetailInfo *saveVehicle = [self.vehicleDetailInfo copy];
        saveVehicle.vehicleNo = vehicleNum;
        saveVehicle.vehicleVin = self.vehicleVin;
        saveVehicle.obdSN = self.obdSN;
        saveVehicle.vehicleBrand = vehicleBrand;
        saveVehicle.vehicleBrandId = (NSString *)_textField2_1.addtionalInfo;
        saveVehicle.vehicleModel = vehicleModel;
        saveVehicle.vehicleModelId = (NSString *)_textField2_2.addtionalInfo;
        saveVehicle.engineNo = _engineText.textField.text;
        saveVehicle.registNo = _registText.textField.text;
        saveVehicle.nextMaintainMileage = nilOrString(_textField4.textField.text);
        saveVehicle.nextInsuranceTime = insuranceDate ? [NSString stringWithFormat:@"%lld",(long long)[insuranceDate timeIntervalSince1970WithMillisecond]] : nil;
        saveVehicle.nextExamineTime = examineDate ? [NSString stringWithFormat:@"%lld",(long long)[examineDate timeIntervalSince1970WithMillisecond]] : nil;
        saveVehicle.currentMileage = currentMile;
        
        KKTBVehicle *tbVehicle = [[KKTBVehicle alloc] initWithDB:[KKDB sharedDB]];
        if(saveVehicle.localId == 0)
        {
            //新增
            if(![tbVehicle existVehicle:saveVehicle])
            {
                if(![tbVehicle existVehicleVin:saveVehicle])
                {
                    if([tbVehicle insertVehicle:saveVehicle])
                    {
                        NSArray *array =[tbVehicle getVehicleWithUserNo:[KKPreference sharedPreference].userInfo.userNo];
                        if([array count] == 1)
                        {
                            KKModelVehicleDetailInfo *info = [array objectAtIndex:0];
                            [tbVehicle setDefaultVehicleWithUserNo:info.userNo vehicleNo:info.vehicleNo];
                        }
                        ...
                        [KKCustomAlertView showAlertViewWithMessage:@"添加车辆成功！" block:^{
                            [self backButtonClicked];
                        }];
                    }
                    else
                    {
                        [KKCustomAlertView showErrorAlertViewWithMessage:@"添加车辆失败！" block:nil];
                    }
                }
                else
                {
                    [KKCustomAlertView showErrorAlertViewWithMessage:@"此车架号已被使用！" block:nil];
                }
            }
            else
            {
                [KKCustomAlertView showErrorAlertViewWithMessage:@"此车牌已存在！" block:nil];
            }
        }
        else
        {
            //修改
            if([tbVehicle updateVehicle:saveVehicle])
            {
                if(![tbVehicle existVehicleVin:saveVehicle])
                {
                    [KKAppDelegateSingleton detachVehicleListAndObdList:[tbVehicle getVehicleWithUserNo:[KKPreference sharedPreference].userInfo.userNo]];
                    [KKCustomAlertView showAlertViewWithMessage:@"修改车辆信息成功！" block:^{
                        [self backButtonClicked];
                    }];
                }
                else
            ...
            {
                [KKCustomAlertView showErrorAlertViewWithMessage:@"修改车辆信息失败！" block:nil];
            }
        }
        [tbVehicle release];
        [saveVehicle release];
