...
- (void) initComponents
{
    ...
    NSMutableArray *nameArr = [[NSMutableArray alloc] initWithObjects:@"用户名 :",@"设置密码 :",@"手机号 :", nil];
    ...
    point2 = CGPointMake(14, 3*38 + 15 + 3*7 + 24);
...
- (void)nextButtonClicked
{
    ...
    for (int t = 0; t < 3;t++)
    {
        ...
        if (t == 1)
        {
            ...
            else if (![KKHelper KKHElpRegexMatchForPassword:textString])
            {
                [KKCustomAlertView showAlertViewWithMessage:@"密码仅支持数字和字母！"];
                return;
            }
            else if ([textString length] > 20)
            {
                [KKCustomAlertView showAlertViewWithMessage:@"密码最多 20 个字符!"];
                return;
            }
            else
            {
...
//            {
//                [KKCustomAlertView showAlertViewWithMessage:@"密码与确认密码不一致！"];
//                return;
//            }
//        }
...
//    [nextVc release];
    ...
    [[KKProtocolEngine sharedPtlEngine] registerWithUserNo:[self.dataArray objectAtIndex:0]
                                                  password:[self.dataArray objectAtIndex:1]
                                                    mobile:[self.dataArray objectAtIndex:2]
                                                      name:nil
                                                 vehicleNo:nil
                                              vehicleModel:nil
                                            vehicleModelId:nil
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    if (textField.tag == 100 && [textField.text length] > 0)
    {
        ...
        {
            KKCustomTextField *nextTextFieldView = [self findNextTextFieldViewFromView:_mainScrollView WithIndex:12];
...
- (NSNumber *)userRegisterResponse:(NSNumber *)aReqId withObject:(id)aRspObj
{
    ...
    userInfo.mobile = [self.dataArray objectAtIndex:2];
    ...
    switch (proRsp.header.code) {
        case eRsp_succeed:
        {
            ...
            NSArray *array = [tbVehicle getVehicleWithUserNo:nil];
            if([array count] > 0)
            {
                [KKCustomAlertView showAlertViewWithMessage:proRsp.header.desc block:^{
                    KKUploadLocalCarViewController *Vc = [[KKUploadLocalCarViewController alloc] init];
                    Vc.localVehicleList = array;
