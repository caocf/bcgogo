...
//    NSLog(@"cacheurls count=%d", [cacheURLs count]);
    NSUInteger idx;
...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if (idx == NSNotFound)
    {
        ...
        if ([delegate respondsToSelector:@selector(webImageManager:didFailWithError:)])
        {
            NSError *err = [NSError errorWithDomain:@"requestCancel" code:250 userInfo:nil];
            [delegate performSelector:@selector(webImageManager:didFailWithError:) withObject:self withObject:err];
        }
        return;
    }
