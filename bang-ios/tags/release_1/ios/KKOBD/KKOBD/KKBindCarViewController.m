...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getVehicleListInfo) name:@"refreshBindCarsView" object:nil];
...
- (void)updateDefaultVehicleButtonClicked:(id)sender
{
    ...
    KKModelVehicleDetailInfo *vehicleInfo = (KKModelVehicleDetailInfo *)[_dataArray objectAtIndex:button.index];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    KKModelVehicleDetailInfo *vehicleInfo = (KKModelVehicleDetailInfo *)[_dataArray objectAtIndex:indexPath.row];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    bgImageView.backgroundColor = [UIColor clearColor];
    bgImageView.userInteractionEnabled = NO;
    bgImageView.image = [_dataArray count] > 0 ? [UIImage imageNamed:@"bg_setting_bind_cell_down.png"] : [UIImage imageNamed:@"bg_setting_bind_cell_full.png"];
    [view addSubview:bgImageView];
    [bgImageView release];
...
- (NSNumber *)vehicleListResponse:(NSNumber *)aReqId withObject:(id)aRspObj
{
    ...
    [_dataArray addObjectsFromArray:listRsp.KKArrayFieldName(vehicleList,KKModelVehicleDetailInfo)];
    [_mainTableView reloadData];
    
    [KKAppDelegateSingleton.vehicleList removeAllObjects];
    [KKAppDelegateSingleton.vehicleList addObjectsFromArray:listRsp.KKArrayFieldName(vehicleList,KKModelVehicleDetailInfo)];
    
    [KKAppDelegateSingleton.obdList removeAllObjects];
    for (KKModelVehicleDetailInfo *detailInfo in KKAppDelegateSingleton.vehicleList)
    {
        if ([detailInfo.isDefault isEqualToString:@"YES"])
            [KKAppDelegateSingleton setCurrentVehicleInfo:detailInfo];
        
        if ([detailInfo.obdSN length] > 0)
        {
            KKModelObdDetailInfo *obdInfo = [[KKModelObdDetailInfo alloc] init];
            obdInfo.obdSN = detailInfo.obdSN;
            if ([detailInfo.isDefault isEqualToString:@"YES"])
            {
    ...
    if ([KKAppDelegateSingleton.vehicleList count] == 0)
        [KKAppDelegateSingleton setCurrentVehicleInfo:nil];
