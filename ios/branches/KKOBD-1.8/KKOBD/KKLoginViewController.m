...
@implementation UIButton(delBtn)

#define KKUIButton_IndexPathKey      @"KKUIButton_IndexPathKey"

- (void)setIndexPath:(NSIndexPath *)aIndexPath
{
    objc_setAssociatedObject(self, KKUIButton_IndexPathKey, aIndexPath, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSIndexPath *)indexPath
{
    return  objc_getAssociatedObject(self, KKUIButton_IndexPathKey);
}
...
- (void)viewDidLoad
{
    ...
    self.userInfoArray = [[KKPreference sharedPreference] userInfoArray];
...
- (void) initComponents
{
    float orignY = [self getOrignY] + 14;
    ...
    _matchingTableView = [[UITableView alloc] initWithFrame:CGRectMake(bg_inputField.frame.origin.x, bg_inputField.frame.origin.y+_accountTextField.frame.size.height, _accountTextField.frame.size.width, 0)];
    ...
    image = [UIImage imageNamed:@"icon_visitorloginBtn.png"];
    UIButton *visitorButton = [[UIButton alloc] initWithFrame:CGRectMake(0.5*(320 - image.size.width), orignY, 130, image.size.height)];
    [visitorButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [visitorButton setTitle:@"游客" forState:UIControlStateNormal];
    [visitorButton setBackgroundImage:image forState:UIControlStateNormal];
    [visitorButton addTarget:self action:@selector(visitorButtonClicked) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:visitorButton];
    [visitorButton release];
    
    image = [UIImage imageNamed:@"icon_loginBtn.png"];
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(168, orignY, 130, image.size.height)];
    ...
    orignY = currentScreenHeight - 22.5 - 8;
...
-(void) visitorButtonClicked
{
    ...
    [KKAppDelegateSingleton visitorLogin];
}
...
- (void)loginButtonClicked
{
    ...
    [KKProtocolEngine sharedPtlEngine].userName = [NSString stringWithFormat:@"%@",_accountTextField.textField.text];
    [KKProtocolEngine sharedPtlEngine].password = [NSString stringWithFormat:@"%@",_passwordTextField.textField.text];
    
    [[KKProtocolEngine sharedPtlEngine] userLoginWithUser:[NSString stringWithFormat:@"%@",_accountTextField.textField.text] password:[NSString stringWithFormat:@"%@",_passwordTextField.textField.text] platform:CurrentSystemPlatform platformVersion:CurrentSystemVersion mobileModel:[KKHelper platformString] appVersion:KKAppDelegateSingleton.versionStr imageVersion:[KKHelper imageVersion] delegate:self];
}
...
-(NSInteger) tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    {
        for(KKModelPreferenceUserInfo *uInfo in self.userInfoArray)
        {
...
-(UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIButton *delBtn = (UIButton *)[cell viewWithTag:8];
    delBtn.indexPath = indexPath;
    ...
    {
        cell.textLabel.text = ((KKModelPreferenceUserInfo *)[self.userInfoArray objectAtIndex:indexPath.row]).userNo;
    }
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(self.accountTextBuf.length > 0)
    {
        for(KKModelPreferenceUserInfo *info in self.userInfoArray)
        {
    ...
    {
        uInfo = [self.userInfoArray objectAtIndex:indexPath.row];
    }
    if(uInfo!=nil)
    {
...
//    }
    ...
    KKModelPreferenceUserInfo *uInfo = [self.userInfoArray objectAtIndex:btn.indexPath.row];
    
    if([_accountTextField.textField.text isEqualToString:uInfo.userNo])
    {
        _accountTextField.textField.text = @"";
...
- (NSNumber *)userLoginResponse:(NSNumber *)aReqId withObject:(id)aRspObj
{
    ...
    [KKAuthorization sharedInstance].authorizationType = Authorization_Register;
