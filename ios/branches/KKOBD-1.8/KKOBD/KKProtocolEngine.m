...
static const KKPtlApiDictionary k_KKPtlApiDict[] = {
    ...
    { @"%@/api/vehicle/multiFault",                 ePtlApi_vehicle_multiFault },    
    ...
    { @"%@/api/shop/guest/comment/list",            ePtlApi_shop_commentList },              // 根据店铺ID获取店铺评论
...
//        domain = [[NSString alloc] initWithString:@"http://192.168.1.90:8080"];
...
- (KKHTTPRequest *) basicRequest:(KKHTTPRequestType)requestType url:(NSString*)url URLEncode:(BOOL)isEncode apiId:(NSInteger)apiId delegate:(id)aDelegate
{
    ...
    {
        NSDictionary *cookieProperties = [NSDictionary dictionaryWithContentsOfFile:[KKHelper getPathWithinDocumentDir:@"cookie"]];
        self.sessionCookie = [NSHTTPCookie cookieWithProperties:cookieProperties];
    }
...
- (void) issueRequest:(KKHTTPRequest*)request
{
    ...
    NSInteger   apiID = [self requestApiID:request];
    BOOL    returnFlag = NO;     //是否跳过请求?
    BOOL doDispatch = NO;            //doDispatch = NO; 不执行delegate
    id retObj = [NSNull null];
    switch (apiID) {
        case ePtlApi_user_login:
        {
            if(self.currentNetworkStatus == kNotReachable)
            {
                jsonStr = [[KKCacheDBManager sharedInstance] getCacheJSONDataWithUserNo:self.userName URL:[request.url description]];
            }
        ...
        {
            //不用存储用户名
            jsonStr = [[KKCacheDBManager sharedInstance] getCacheJSONDataWithUserNo:nil URL:[request.url description]];
        }
            break;
        ...
        {
            jsonStr = [[KKCacheDBManager sharedInstance] getCacheJSONDataWithUserNo:self.userName URL:[self getAccessUrl:apiID]];
        }
            break;
        default:
            break;
    ...
    if(jsonStr)
    {
        ...
        if (parser)
            retObj = [parser parse:[jsonStr mutableObjectFromJSONString]];
    }
    
    doDispatch = [((NSNumber *)[self localCacheGetSuccess:request cacheObject:retObj]) boolValue];
    
    if(jsonStr && doDispatch)
    {
        SEL sel = [KKProtocolEngineSelector getResponseSEL:apiID];
        if (sel)
            [self DispatchResponse:request withObject:retObj action:sel loadCache:YES];
    }
...
-(id) localCacheGetSuccess:(KKHTTPRequest*)request cacheObject:(id)cacheObj
{
	NSObject *delegateObject = (NSObject*)[self requestDelegate:request];
    NSInteger aReqId = [self requestApiID:request];
	NSNumber *reqIdNumber = [NSNumber numberWithInteger:aReqId];
    id x=[NSNumber numberWithBool:YES];
    SEL aSelector = @selector(localCacheGetSuccess:withObject:);
    if (delegateObject) {
		if ([delegateObject conformsToProtocol:@protocol(KKProtocolEngineDelegate)] && [delegateObject respondsToSelector:aSelector])
			x = [delegateObject performSelector:aSelector withObject:reqIdNumber withObject:cacheObj];
		//return YES;
	}
    return x;
}
...
- (void) DispatchResponse:(KKHTTPRequest*)request withObject:(id)anObject action:(SEL)aSelctor
{
    [self DispatchResponse:request withObject:anObject action:aSelctor loadCache:NO];
}

- (void) DispatchResponse:(KKHTTPRequest*)request withObject:(id)anObject action:(SEL)aSelctor loadCache:(BOOL)aLoadCache
{
	NSInteger reqID = [self requestID:request];
	NSNumber *reqIdNumber = aLoadCache ? [NSNumber numberWithInteger:ePtlApi_LoadCache] : [NSNumber numberWithInteger:reqID];
	NSObject *delegateObject = (NSObject*)[self requestDelegate:request];
...
-(uint32_t) currentNetworkStatus
{
    return [ASIHTTPRequest currentNetworkStatus];
}
...
- (NSInteger)vehicleMultiFault:(NSArray *)faultArray delegate:(id)aDelegate
{
    NSString *url = [self getAccessUrl:ePtlApi_vehicle_multiFault];
    
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithCapacity:20];
    int i = 0;
    for(KKModelDTCMessage *message in faultArray)
    {
        NSString *str_vehicleId = [NSString stringWithFormat:@"vehicleFaults[%d].vehicleId",i];
        NSString *str_faultCode = [NSString stringWithFormat:@"vehicleFaults[%d].faultCode",i];
        NSString *str_obdSN = [NSString stringWithFormat:@"vehicleFaults[%d].obdSN",i];
        NSString *str_reportTime = [NSString stringWithFormat:@"vehicleFaults[%d].reportTime",i];
        KKSetParamString(params, str_vehicleId, KKAppDelegateSingleton.currentVehicle.vehicleId, NO);
        KKSetParamString(params, str_faultCode, message.faultCode, NO);
        KKSetParamString(params, str_obdSN, KKAppDelegateSingleton.currentVehicle.obdSN, NO);
        KKSetParamString(params, str_reportTime, message.warnTimeStamp, NO);
        i++;
    }
    
    KKFormDataRequest *request = (KKFormDataRequest*)[self basicRequest:eHTTPFormRequest url:url apiId:ePtlApi_vehicle_multiFault delegate:aDelegate];
    [KKProtocolEngine makePostData:params request:request];
	[self issueRequest:request];
...
- (NSInteger)shopCommentListWithId:(NSString *)aShopId pageNo:(NSInteger)aPageNo pageSize:(NSInteger)aPageSize delegate:(id)aDelegate
{
    
    NSString *url = [self getAccessUrl:ePtlApi_shop_commentList];
    NSAssert(url, @"shop detail url can't get");
...
//    }
    ...
    KKSetSequenceParamInt(seqKeys, params, @"pageSize", aPageSize);
    reqUrl = [NSString stringWithFormat:@"%@%@", url, [KKProtocolEngine makeURIParams:params sequenceKeys:seqKeys containKeys:NO]];
    KKHTTPRequest *request = (KKHTTPRequest*)[self basicRequest:eHTTPRequest url:reqUrl apiId:ePtlApi_shop_commentList delegate:aDelegate];
	[self issueRequest:request];
...
//    NSLog(@"cookies are %@", cookies);
    for (NSHTTPCookie *cookie in cookies) {
        ...
        {
            if(![self.sessionCookie.value isEqualToString:cookie.value])
            {
                NSDictionary *cookieProperties = cookie.properties;
                [cookieProperties writeToFile:[KKHelper getPathWithinDocumentDir:@"cookie"] atomically:YES];
            }
        ...
        if (nil == jsonObj || NO == [[jsonObj class] isSubclassOfClass:[NSMutableDictionary class]]) {
            ...
            else
            {
                switch (apiID) {
                    ...
                    case ePtlApi_vehicle_list:
                    {
                        [[KKCacheDBManager sharedInstance] insertJSONDataWithUserNo:self.userName URL:request.url.description jsonData:responseString];
                    }
                        break;
                    case ePtlApi_area_list:
                    case ePtlApi_serviceCategory_list:
                    case ePtlApi_violate_juheAreaList:
                    {
                        [[KKCacheDBManager sharedInstance] insertJSONDataWithUserNo:nil URL:request.url.description jsonData:responseString];
                    }
                        break;
                    case ePtlApi_shop_searchList:
                    case ePtlApi_service_historyList:
                    ...
                    {
                        BOOL isSave = NO;
                        if([retObj class] == [KKModelShopSearchListRsp class])
                        {
                            if(((KKModelShopSearchListRsp *)retObj).pager.currentPage == 1)
                            {
                                isSave = YES;
                            }
                        }
                        if([retObj class] == [KKModelServiceHistoryListRsp class])
                        {
                            if(((KKModelServiceHistoryListRsp *)retObj).pager.currentPage == 1)
                            {
                                isSave = YES;
                            }
                        }
                        if([retObj class] == [KKViolateResultRsp class])
                        {
                            isSave = YES;
                        }
                        if(isSave)
                            [[KKCacheDBManager sharedInstance] insertJSONDataWithUserNo:self.userName URL:[self getAccessUrl:apiID] jsonData:responseString];
                    }
                        break;
                    case ePtlApi_vehicle_getModelByKey:
                    {
                        BOOL isSave = NO;
                        if([request.url.description rangeOfString:@"brandModel/keywords/NULL/type/brand/brandId/NULL"].length > 0)
                        {
                            //热门品牌
                            isSave = YES;
                        }
                        ...
                        {
                            if([request.url.description rangeOfString:@"/type/brand/brandId/NULL"].length == 0)
                            {
                                //车型
                                isSave = YES;
                            }
