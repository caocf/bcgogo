...
static uint32_t     currentNetworkStatus;
...
- (void)main
{
	@try {
		...
		for (header in [self requestHeaders]) {
            ...
            NSLog(@"request header: %@:%@", header, [[self requestHeaders] objectForKey:header]);
		}
...
{
    currentNetworkStatus = [[Reachability reachabilityForInternetConnection] currentReachabilityStatus];
	return (currentNetworkStatus == ReachableViaWWAN);
}
