...
-(void) submitBtnClicked
{
    ...
    if([KKAuthorization sharedInstance].accessAuthorization.localCarManager)
    {
        ...
        saveVehicle.vehicleVin = _classaText.textField.text;
        ...
        if(![tbVehicle existVehicleVin:saveVehicle])
        {
            ...
            {
                [KKCustomAlertView showAlertViewWithMessage:@"车辆信息保存成功！" block:^{
            ...
            {
                [KKCustomAlertView showErrorAlertViewWithMessage:@"车辆信息保存失败！" block:nil];
            }
        }
        else
        {
