...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([KKAuthorization sharedInstance].accessAuthorization.localCarManager)
    {
        ...
        if([tbVehicle deleteVehicle:vehicleInfo])
        {
            ...
            {
                if([info.isDefault isEqualToString:@"YES"])
                {
                    setDefault = NO;
                    break;
                }
            }
            if(setDefault && [KKAppDelegateSingleton.vehicleList count] > 0)
            {
                KKModelVehicleDetailInfo *info = [KKAppDelegateSingleton.vehicleList objectAtIndex:0];
...
- (NSNumber *)vehicleListResponse:(NSNumber *)aReqId withObject:(id)aRspObj
{
    ...
    [KKAppDelegateSingleton setDefaultOilPrice:listRsp.defaultOilPrice oilKind:listRsp.defaultOilKind];
