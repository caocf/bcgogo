...
{
    if(_vehicleNoText && _vehicleNoText.textField.text.length == 0)
    {
...
//    return;
    ...
    for(KKViolateCityInfo *cInfo in self.currentCities)
    {
        ...
        if(self.currentVehicle.engineNo && cInfo.juheViolateRegulationCitySearchCondition__KKViolateSearchCondition.engine)
        {
            ...
            {
                engineNoStr = [self.currentVehicle.vehicleVin substringFromIndex:(self.currentVehicle.engineNo.length-cInfo.juheViolateRegulationCitySearchCondition__KKViolateSearchCondition.engineNo)];
            }
        }
