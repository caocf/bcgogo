...
static NSInteger staticMemCacheMaxItems = 10;   // 10 items
static NSInteger cacheMaxCacheAge = 60*60*24*7; // 1 week

static SDImageCache *instance;

@interface SDImageCache(_private)
- (void) setMemCacheObject:(id)object forKey:(id)key;
@end

@implementation SDImageCache
@synthesize memCacheMaxItems;
...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        memCacheKeys = [[NSMutableArray alloc] initWithCapacity:10];
        memCacheMaxItems = staticMemCacheMaxItems;
...
- (void)dealloc
{
    ...
    [memCacheKeys release], memCacheKeys = nil;
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    if (image)
    {
        [self setMemCacheObject:image forKey:key];
...
- (void) setMemCacheObject:(id)object forKey:(id)key
{
    NSInteger count = [memCache count];
    if (count >= self.memCacheMaxItems && nil == [memCache objectForKey:key]) {
        [memCache removeObjectForKey:[memCacheKeys objectAtIndex:0]];
        [memCacheKeys removeObjectAtIndex:0];
    }
    [memCache setObject:object forKey:key];
    [memCacheKeys addObject:key];
}
