...
- (void)setNavgationBar
{
    ...
    [self.navigationItem setRightBarButtonItems:[NSArray arrayWithObjects:[KKViewUtils createNavigationBarButtonItem:[UIImage imageNamed:@"icon_shopq_map.png"] bgImage:nil target:self action:@selector(mapButtonClicked)],[KKViewUtils createNavigationBarButtonItemWithTitle:@"更多" bgImage:nil target:self action:@selector(moreShopButtonClicked)],nil]];
}
...
- (void)setCityDataAndShow
{
    ...
    for (int i = 0 ;i < [_provinceArray count] ; i++)
    {
        ...
        if ([areaInfo.name isEqualToString:_currentCity.provinceName])
            topSelected = i;
