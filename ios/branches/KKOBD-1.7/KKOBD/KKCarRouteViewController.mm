...
@interface KKCarRouteViewController ()

@end

@implementation KKCarRouteViewController
...
- (void)viewDidLoad
{
    ...
    NSArray *arr = [KKHelper getArray:self.shopInfo.coordinate BySeparateString:@","];
    ...
    pointAnn.coordinate = coo;
    pointAnn.title = self.shopInfo.name;
    pointAnn.subtitle = self.shopInfo.address;
...
- (void) initComponents
{
    ...
    _mapView = [[BMKMapView alloc] initWithFrame:CGRectMake(0, 0, 320, currentScreenHeight - 44 - 49 - [self getOrignY])];
    _mapView.delegate = self;
...
-(void)mapView:(BMKMapView *)mapView didUpdateUserLocation:(BMKUserLocation *)userLocation
{
    if (!Center)
    {
        ...
        end.pt = CLLocationCoordinate2DMake([arr[1] doubleValue], [arr[0] doubleValue]);
        end.name = nil;
