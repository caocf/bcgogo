...
- (BOOL)updateDriveRecord:(TGModelDriveRecordDetail *)aDetail
{
    ...
    {
        NSString *sql = [NSString stringWithFormat:@"update %@ set appUserNo=?,lastUpdateTime=?,vehicleNo=?,startTime=?,startLat=?,startLon=?,startPlace=?,endTime=?,endLat=?,endLon=?,endPlace=?,travelTime=?,distance=?,oilCost=?,oilWear=?,oilPrice=?,oilKind=?,totalOilMoney=?,appPlatform=?,status=? where id=?",TGTABLENAME];
        @try {
            [self.db executeUpdate:sql,
             aDetail.appUserNo,
             ...
             aDetail.appPlatform,
             aDetail.status,
             [NSNumber numberWithLongLong:aDetail.id]];
