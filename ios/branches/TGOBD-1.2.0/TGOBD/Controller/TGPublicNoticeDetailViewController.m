    ...
    [[TGHTTPRequestEngine sharedInstance] getAdvertDetail:_advertId viewControllerIdentifier:self.viewControllerIdentifier success:^(AFHTTPRequestOperation *operation, id responseObject) {
        if ([self httpResponseCorrect:responseObject]) {
            ...
            if ([rsp.advertDTOList__TGModelPublicNoticeInfo count] > 0) {
                ...
                NSString *publishDate = [NSDate dateStringWithTimeIntervalSince1970WithMillisecond:[[rsp.advertDTOList__TGModelPublicNoticeInfo objectAtIndex:0] publishDate] formatter:nil];
                NSString *beginDate = [[rsp.advertDTOList__TGModelPublicNoticeInfo objectAtIndex:0] beginDateStr];
                NSString *endDate = [[rsp.advertDTOList__TGModelPublicNoticeInfo objectAtIndex:0] endDateStr];
                
                NSMutableString *time = [[NSMutableString alloc] init];
                
                beginDate.length == 0 ? [time appendString:publishDate] : [time appendString:beginDate];
                [time appendString:@" 至 "];
                endDate.length == 0 ? [time appendString:@"永久"] : [time appendString:endDate];
