...
@property (nonatomic, assign) BOOL ifRemberPwd;
@end
...
//    [scrollView setFrame:CGRectMake(0, originY, screenWidth, height)];
    ...
    forgetText.returnKeyType = UIReturnKeySend;
    forgetText.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
    [forgetText setValue:[NSNumber numberWithInt:15] forKey:systemVersionAboveiOS7 ? @"paddingLeft" : @"_paddingLeft"];
    ...
    [submitForgetBtn setBackgroundImage:[[UIImage imageNamed:@"bg_button_blue.png"] resizableImageWithCapInsets:UIEdgeInsetsMake(25, 102, 25, 102) resizingMode:UIImageResizingModeStretch] forState:UIControlStateNormal];
    ...
    //设置是否记住密码
    _ifRemberPwd = [[NSUserDefaults standardUserDefaults] boolForKey:USER_REMBER_PASSWORD];
    if (_ifRemberPwd) {
        [remeberPwd setBackgroundImage:[UIImage imageNamed:@"on.png"] forState:UIControlStateNormal];
        passwordText.text = [[NSUserDefaults standardUserDefaults] objectForKey:USER_PASSWORD];
    } else
    {
        [remeberPwd setBackgroundImage:[UIImage imageNamed:@"off.png"] forState:UIControlStateNormal];
    }
...
- (IBAction)loginButtonClicked:(id)sender {
    ...
    if (![TGHelper isValidateMobile:userNoText.text]) {
        [TGAlertView showAlertViewWithTitle:nil message:@"请输入正确的手机号码"];
        return;
    }
    
    if (passwordText.text.length == 0) {
        [TGAlertView showAlertViewWithTitle:nil message:@"请输入密码"];
        return;
    }
    ...
    [[TGHTTPRequestEngine sharedInstance] userLogin:userNoText.text password:passwordText.text viewControllerIdentifier:self.viewControllerIdentifier success:^(AFHTTPRequestOperation *operation, id responseObject) {
        if([self httpResponseCorrect:responseObject])
        {
            ...
            [TGDataSingleton sharedInstance].shopInfo = rsp.appShopDTO;
            ...
            [[NSUserDefaults standardUserDefaults] setBool:_ifRemberPwd forKey:USER_REMBER_PASSWORD];
            if (_ifRemberPwd) {
                [[NSUserDefaults standardUserDefaults] setObject:passwordText.text forKey:USER_PASSWORD];
            }
            else
            {
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    } else if (textField == forgetText)
    {
        [self submitForgetButtonClicked];
        [self.view endEditing:YES];
    }
    return YES;
}
