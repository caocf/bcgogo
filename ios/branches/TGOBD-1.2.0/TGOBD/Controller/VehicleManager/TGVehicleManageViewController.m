...
- (void)initComponents
{
    ...
    _vehicleNo = [[TGCustomTextFieldView alloc] initWithFrame:CGRectMake(0, originY, 320, 50) leftTitle:@"车牌号码:" placeholder:nil rightTitle:nil rightImage:[UIImage imageNamed:@"icon_edit.png"]];
    _vehicleNo.textField.enabled = NO;
    
    UIButton *editVehicleNoBtn = [[UIButton alloc] initWithFrame:_vehicleNo.bounds];
    editVehicleNoBtn.backgroundColor = [UIColor clearColor];
    [editVehicleNoBtn addTarget:self action:@selector(editVehicleNo) forControlEvents:UIControlEventTouchUpInside];
    [_vehicleNo addSubview:editVehicleNoBtn];
...
- (void)editVehicleNo
{
    TGVehicleNoEditViewController *vc = [[TGVehicleNoEditViewController alloc] init];
    vc.delegate = self;
...
- (void)selectCity
{
    ...
    [[TGHTTPRequestEngine sharedInstance] violateGetCityList:self.viewControllerIdentifier success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([rsp.areaList__TGModelViolateCityInfo count] == 0) {
            [TGAlertView showAlertViewWithTitle:nil message:@"当前无法选择城市"];
            return ;
        }
...
{
    NSString *vehicleNo = _vehicleNo.textField.text;
    ...
    {
        [request updateVehicleInfo:oilcePrice vehicleNo:vehicleNo currentMileage:currentMileage maintainPeriod:maintainPeriod juheCityCode:_juheCityCode juheCityName:_queryCity.textField.text registerNo:registerNo engineNo:engineNo vehicleVin:vehicleVin lastMaintainMileage:lastMaintainMileage nextMaintainTime:[NSDate timeIntervalSince1970WithMillisecondFromString:nextMaintainTime formatter:nil] nextExamineTime:[NSDate timeIntervalSince1970WithMillisecondFromString:nextExamineTime formatter:nil] viewControllerIdentifier:self.viewControllerIdentifier success:^(AFHTTPRequestOperation *operation, id responseObject) {
            
            if ([self httpResponseCorrect:responseObject]) {
                dataSingleton.vehicleInfo.vehicleNo = vehicleNo;
...
#pragma mark - TGPassEditVehicleNo delegate

- (void)passEditVehicleN0:(NSString *)vehicleNo
{
    _vehicleNo.textField.text = vehicleNo;
}
