...
{    
    ...
    //配置,是否是正式环境
    [KKProtocolEngine sharedPtlEngine].serviceEnvironment = NO;
...
//    self.rootViewController = nil;
    ...
    [KKDriveRecordEngine sharedInstance].firstRecordTime = [NSDate dateWithTimeIntervalSince1970WithMillisecond:[[[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"%@_firstRecordTime",[KKProtocolEngine sharedPtlEngine].userName]] longLongValue]];
...
- (void)vehicleRealtimeDataResp:(id)aRetObj
{
    ...
    [KKDriveRecordEngine sharedInstance].driveRecordDetail.travelTime = [[NSDate date] timeIntervalSince1970] - [self.obdConnectTime timeIntervalSince1970];
