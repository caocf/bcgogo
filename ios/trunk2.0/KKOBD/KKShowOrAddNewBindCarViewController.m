...
//        }
        self.vehicleDetailInfo.vehicleNo = vehicleNum;
        self.vehicleDetailInfo.vehicleVin = self.vehicleVin;
        self.vehicleDetailInfo.obdSN = self.obdSN;
        self.vehicleDetailInfo.vehicleBrand = vehicleBrand;
        self.vehicleDetailInfo.vehicleBrandId = (NSString *)_textField2_1.addtionalInfo;
        self.vehicleDetailInfo.vehicleModel = vehicleModel;
        self.vehicleDetailInfo.vehicleModelId = (NSString *)_textField2_2.addtionalInfo;
        self.vehicleDetailInfo.engineNo = _engineText.textField.text;
        self.vehicleDetailInfo.registNo = _registText.textField.text;
        self.vehicleDetailInfo.nextMaintainMileage = nilOrString(_textField4.textField.text);
        self.vehicleDetailInfo.nextInsuranceTime = insuranceDate ? [NSString stringWithFormat:@"%ld",(long)[insuranceDate timeIntervalSince1970]] : nil;
        self.vehicleDetailInfo.nextExamineTime = examineDate ? [NSString stringWithFormat:@"%ld",(long)[examineDate timeIntervalSince1970]] : nil;
        self.vehicleDetailInfo.currentMileage = currentMile;
        
        KKTBVehicle *tbVehicle = [[KKTBVehicle alloc] initWithDB:[KKDB sharedDB]];
        if(self.vehicleDetailInfo.localId == 0)
        {
            //新增
            if(![tbVehicle existVehicle:self.vehicleDetailInfo])
            {
                if([tbVehicle insertVehicle:self.vehicleDetailInfo])
                {
        ...
        {
            //修改
            if([tbVehicle updateVehicle:self.vehicleDetailInfo])
            {
                [KKAppDelegateSingleton detachVehicleListAndObdList:[tbVehicle getVehicleWithUserNo:[KKPreference sharedPreference].userInfo.userNo]];
