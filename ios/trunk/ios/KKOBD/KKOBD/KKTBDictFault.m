...
- (BOOL)createTableWithVehicleModel:(NSString *)aModel dictDetail:(KKModelVehicleFaultDictRsp*)aDict;
{
    ...
    @try {
        ...
        for (KKModelFaultCodeInfo *info in dicts) {
            NSString *sqlFmt = @"INSERT INTO %@ VALUES('%@', '%@')";
            NSString *sql = [NSString stringWithFormat:sqlFmt, tbName, info.faultCode, info.description];
...
- (NSArray *)getFaultInfoInTable:(NSString *)aTableName code:(NSString *)aCode
{
    ...
    @try {
        NSString *sql = [NSString stringWithFormat:@"SELECT code, description FROM %@ WHERE code='%@'", aTableName, aCode];
        FMResultSet *rs = [_db executeQuery:sql];
        while ([rs next]) {
